...
int rabin_next_chunk(rabin_t *h, uint8_t *buf, uint32_t len, uint32_t min_size,
                     uint32_t max_size) {
    FOUNDATION_ASSERT(min_size < max_size);
    FOUNDATION_ASSERT(min_size >= 4 * 1024 * 1024);
    FOUNDATION_ASSERT(max_size <= 32 * 1024 * 1024);
    ...
    for (unsigned int i = 0; i < len; i++) {
        ...
        if ((h->count >= min_size && ((h->digest & MASK) == 0)) ||
            h->count >= max_size) {
...
void rabin_deallocate(rabin_t *h) { memory_deallocate(h); }