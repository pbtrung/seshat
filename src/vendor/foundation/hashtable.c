...
void hashtable32_initialize(hashtable32_t *table, size_t buckets) {
    memset(table, 0,
           sizeof(hashtable32_t) + sizeof(hashtable32_entry_t) * buckets);
...
bool hashtable32_set(hashtable32_t *table, uint32_t key, uint32_t value) {
    /*lint --e{613} */
    size_t ie, eend;
    ...
    do {
        ...
        if ((current_key == key) ||
            (!current_key &&
             atomic_cas32(&table->entries[ie].key, (int32_t)key, 0,
...
size_t hashtable32_size(hashtable32_t *table) {
    ...
    for (ie = 0; ie < table->capacity; ++ie) {
        if (atomic_load32(&table->entries[ie].key, memory_order_consume) &&
            table->entries[ie].value)
            ++count;
    }
    return count;
}
...
void hashtable64_initialize(hashtable64_t *table, size_t buckets) {
    memset(table, 0,
           sizeof(hashtable64_t) + sizeof(hashtable64_entry_t) * buckets);
    table->capacity = buckets;
}
...
bool hashtable64_set(hashtable64_t *table, uint64_t key, uint64_t value) {
    size_t ie, eend;
...
size_t hashtable64_size(hashtable64_t *table) {
    ...
    size_t ie;
    for (ie = 0; ie < table->capacity; ++ie) {
        if (atomic_load64(&table->entries[ie].key, memory_order_consume) &&
            table->entries[ie].value)
            ++count;
    }
    return count;
