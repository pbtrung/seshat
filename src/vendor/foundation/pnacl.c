/* pnacl.c  -  Foundation library  -  Public Domain  -  2013 Mattias Jansson /
 * Rampant Pixels
 *
...
string_const_t pnacl_error_message(int err) {
    switch (err) {
    case PP_OK:
        return string_const(STRING_CONST("ok"));
    case PP_OK_COMPLETIONPENDING:
        return string_const(STRING_CONST("completion pending"));
    case PP_ERROR_FAILED:
        return string_const(STRING_CONST("failed"));
    case PP_ERROR_ABORTED:
        return string_const(STRING_CONST("aborted"));
    case PP_ERROR_BADARGUMENT:
        return string_const(STRING_CONST("bad argument"));
    case PP_ERROR_BADRESOURCE:
        return string_const(STRING_CONST("bad resource"));
    case PP_ERROR_NOINTERFACE:
        return string_const(STRING_CONST("no interface"));
    case PP_ERROR_NOACCESS:
        return string_const(STRING_CONST("no access"));
    case PP_ERROR_NOMEMORY:
        return string_const(STRING_CONST("no memory"));
    case PP_ERROR_NOSPACE:
        return string_const(STRING_CONST("no space"));
    case PP_ERROR_NOQUOTA:
        return string_const(STRING_CONST("no quota"));
    case PP_ERROR_INPROGRESS:
        return string_const(STRING_CONST("in progress"));
    case PP_ERROR_NOTSUPPORTED:
        return string_const(STRING_CONST("not supported"));
    case PP_ERROR_BLOCKS_MAIN_THREAD:
        return string_const(STRING_CONST("blocks main thread"));
    case PP_ERROR_MALFORMED_INPUT:
        return string_const(STRING_CONST("malformed input"));
    case PP_ERROR_RESOURCE_FAILED:
        return string_const(STRING_CONST("resource failed"));
    case PP_ERROR_FILENOTFOUND:
        return string_const(STRING_CONST("file not found"));
    case PP_ERROR_FILEEXISTS:
        return string_const(STRING_CONST("file exists"));
    case PP_ERROR_FILETOOBIG:
        return string_const(STRING_CONST("file too big"));
    case PP_ERROR_FILECHANGED:
        return string_const(STRING_CONST("file changed"));
    case PP_ERROR_NOTAFILE:
        return string_const(STRING_CONST("not a file"));
    case PP_ERROR_TIMEDOUT:
        return string_const(STRING_CONST("timeout"));
    case PP_ERROR_USERCANCEL:
        return string_const(STRING_CONST("user cancel"));
    case PP_ERROR_NO_USER_GESTURE:
        return string_const(STRING_CONST("no user gesture"));
    case PP_ERROR_CONTEXT_LOST:
        return string_const(STRING_CONST("context lost"));
    case PP_ERROR_NO_MESSAGE_LOOP:
        return string_const(STRING_CONST("no message loop"));
    case PP_ERROR_WRONG_THREAD:
        return string_const(STRING_CONST("wrong thread"));
    case PP_ERROR_WOULD_BLOCK_THREAD:
        return string_const(STRING_CONST("would block thread"));
    case PP_ERROR_CONNECTION_CLOSED:
        return string_const(STRING_CONST("connection closed"));
    case PP_ERROR_CONNECTION_RESET:
        return string_const(STRING_CONST("connection reset"));
    case PP_ERROR_CONNECTION_REFUSED:
        return string_const(STRING_CONST("connection refused"));
    case PP_ERROR_CONNECTION_ABORTED:
        return string_const(STRING_CONST("connection aborted"));
    case PP_ERROR_CONNECTION_FAILED:
        return string_const(STRING_CONST("connection failed"));
    case PP_ERROR_CONNECTION_TIMEDOUT:
        return string_const(STRING_CONST("connection timeout"));
    case PP_ERROR_ADDRESS_INVALID:
        return string_const(STRING_CONST("address invalid"));
    case PP_ERROR_ADDRESS_UNREACHABLE:
        return string_const(STRING_CONST("address unreachable"));
    case PP_ERROR_ADDRESS_IN_USE:
        return string_const(STRING_CONST("address in use"));
    case PP_ERROR_MESSAGE_TOO_BIG:
        return string_const(STRING_CONST("message too big"));
    case PP_ERROR_NAME_NOT_RESOLVED:
        return string_const(STRING_CONST("name not resolved"));
    default:
        break;
    }
...
const void *pnacl_interface(const char *interface, size_t length) {
    string_t interfacestr = string_thread_buffer();
    interfacestr = string_copy(STRING_ARGS(interfacestr), interface, length);
    return _pnacl_browser_interface ? _pnacl_browser_interface(interfacestr.str)
           : 0;
}
...
static char *pnacl_strdup(const char *str) {
...
static PP_Bool pnacl_instance_create(PP_Instance instance, uint32_t argc,
    ...
    int err = pthread_create(&main_thread, 0, pnacl_instance_main_thread,
    ...
    if (err) {
        ...
        log_errorf(
            ...
            STRING_CONST(
                "Unable to create main thread: pthread_create failed: %.*s"),
...
static void pnacl_instance_change_focus(PP_Instance instance,
                                        PP_Bool has_focus) {
    FOUNDATION_UNUSED(instance);
    FOUNDATION_UNUSED(has_focus);
}

static PP_Bool pnacl_instance_document_load(PP_Instance instance,
...
const void *pnacl_module_interface(const char *interface, size_t length) {
    if (string_equal(interface, length, STRING_CONST(PPP_INSTANCE_INTERFACE)) ==
        0) {
        static PPP_Instance instance_interface = {
            &pnacl_instance_create, &pnacl_instance_destroy,
            &pnacl_instance_change_view, &pnacl_instance_change_focus,
...
void *pnacl_array_output(void *arr, uint32_t count, uint32_t size) {
    ...
    if (size) {
        array->data =
            memory_allocate(HASH_PNACL, count * size, 0,
...
void pnacl_post_log(hash_t context, error_level_t severity, const char *msg,
                    size_t msglen) {
    ...
    jsonmsg = string_allocate_format(
                  STRING_CONST("{\"type\":\"log\",\"context\":\"%" PRIx64
                               "\",\"severity\":\"%d\",\"msg\":\"%.*s\"}"),
                  context, severity, STRING_FORMAT(cleanmsg));
    string_replace(jsonmsg.str + (jsonmsg.length - (cleanmsg.length + 2)),
                   cleanmsg.length, cleanmsg.length, STRING_CONST("\""),
                   STRING_CONST("'"), false);
