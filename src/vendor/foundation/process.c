...
process_t *process_allocate(void) {
    process_t *proc =
        memory_allocate(0, sizeof(process_t), 0, MEMORY_PERSISTENT);
...
void process_set_working_directory(process_t *proc, const char *path,
    ...
    if (proc->wd.length <= length)
        proc->wd = string_resize(proc->wd.str, proc->wd.length,
                                 proc->wd.length ? proc->wd.length + 1 : 0,
                                 length + 1, 0);
...
void process_set_arguments(process_t *proc, const string_const_t *args,
                           size_t num) {
    size_t ia;
    string_array_deallocate(proc->args);
    for (ia = 0; ia < num; ++ia)
...
void process_set_verb(process_t *proc, const char *verb, size_t length) {
    ...
    if (proc->verb.length <= length)
        proc->verb = string_resize(
                         proc->verb.str, proc->verb.length,
...
int process_spawn(process_t *proc) {
    static const string_const_t unescaped = {
        STRING_CONST("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234"
                     "56789-_.:/\\")
    };
    size_t i, num_args;
    size_t size;
...
#if FOUNDATION_PLATFORM_POSIX
    if (string_find_first_not_of(STRING_ARGS(proc->path),
                                 STRING_ARGS(unescaped), 0) != STRING_NPOS)
#endif
    {
        bool preesc = (proc->path.str[0] != '"');
        bool postesc = (proc->path.str[proc->path.length - 1] != '"');
        if (preesc || postesc) {
            char *buffer = memory_allocate(HASH_STRING, proc->path.length + 4,
                                           0, MEMORY_PERSISTENT);
            string_t pathesc = string_concat_varg(
                                   buffer, proc->path.length + 4, "\"", (size_t)(preesc ? 1 : 0),
                                   STRING_ARGS(proc->path), "\"", (size_t)(postesc ? 1 : 0),
                                   nullptr);
            string_deallocate(proc->path.str);
            proc->path = pathesc;
        }
...
#if !FOUNDATION_PLATFORM_POSIX
        if (string_find_first_not_of(arg.str, arg.length, unescaped.str,
                                     unescaped.length, 0) != STRING_NPOS) {
            if (arg.str[0] != '"') {
                ...
                while (pos != STRING_NPOS) {
                    if (arg.str[pos - 1] != '\\') {
                        string_const_t right =
                            string_substr(STRING_ARGS(arg), 0, pos);
                        string_const_t left =
                            string_substr(STRING_ARGS(arg), pos, STRING_NPOS);
                        capacity = arg.length + 2;
                        escarg = string_allocate(0, capacity);
                        escarg = string_concat(escarg.str, capacity,
                                               STRING_ARGS(right),
                                               STRING_CONST("\\"));
                        escarg = string_append(STRING_ARGS(escarg), capacity,
                                               STRING_ARGS(left));
                ...
                escarg = string_allocate_concat_varg(
                             STRING_CONST("\""), STRING_ARGS(arg), STRING_CONST("\""),
                             nullptr);
                string_deallocate(arg.str);
                proc->args[i] = escarg;
            }
...
#define SEE_MASK_NOASYNC 0x00000100
    ...
    for (i = 0; i < size; ++i) {
        ...
        if (cmdline.length)
            cmdline = string_append(STRING_ARGS(cmdline), capacity,
                                    STRING_CONST(" "));
        cmdline =
            string_append(STRING_ARGS(cmdline), capacity, STRING_ARGS(arg));
    }

    if (!proc->wd.length)
        proc->wd =
            string_clone(STRING_ARGS(environment_current_working_directory()));
    ...
    if (proc->flags & PROCESS_WINDOWS_USE_SHELLEXECUTE) {
        ...
        wverb = (proc->verb.length
                 ? wstring_allocate_from_string(STRING_ARGS(proc->verb))
                 : 0);
        ...
        if (proc->flags & PROCESS_STDSTREAMS)
            log_warn(0, WARNING_UNSUPPORTED,
                     STRING_CONST("Unable to redirect standard in/out"
                                  " through pipes when using ShellExecute for "
                                  "process spawning"));

        // log_debugf(0, STRING_CONST("Spawn process (ShellExecute): %.*s
        // %.*s"),
        ...
        if (!ShellExecuteExW(&sei)) {
            ...
            log_warnf(0, WARNING_SYSTEM_CALL_FAIL,
                      STRING_CONST("Unable to spawn process (ShellExecute) for "
                                   "executable '%.*s': %.*s"),
        ...
        if (proc->flags & PROCESS_STDSTREAMS) {
            ...
            SetHandleInformation(pipe_read_handle(proc->pipeout),
                                 HANDLE_FLAG_INHERIT, 0);
            SetHandleInformation(pipe_read_handle(proc->pipeerr),
                                 HANDLE_FLAG_INHERIT, 0);
            SetHandleInformation(pipe_write_handle(proc->pipein),
                                 HANDLE_FLAG_INHERIT, 0);

            inherit_handles = TRUE;
        }

        // log_debugf(0, STRING_CONST("Spawn process (CreateProcess): %.*s
        // %.*s"),
        //           STRING_FORMAT(proc->path), STRING_FORMAT(cmdline));

        if (!CreateProcessW(0, wcmdline, 0, 0, inherit_handles,
                            (proc->flags & PROCESS_CONSOLE) ? CREATE_NEW_CONSOLE
                            : 0,
                            0, wwd, &si, &pi)) {
            string_const_t errstr = system_error_message(0);
            log_warnf(0, WARNING_SYSTEM_CALL_FAIL,
                      STRING_CONST("Unable to spawn process (CreateProcess) "
                                   "for executable '%.*s': %.*s"),
                      STRING_FORMAT(proc->path), STRING_FORMAT(errstr));
    ...
    if (proc->flags & PROCESS_MACOS_USE_OPENAPPLICATION) {
        ...
        for (i = 0, size = array_size(proc->args); i < size;
            ...
            array_push(args, CFStringCreateWithCString(0, proc->args[i].str,
                       kCFStringEncodingUTF8));

        CFArrayRef argvref =
            CFArrayCreate(0, (const void **)args, (CFIndex)array_size(args), 0);

        params.flags = kLSLaunchDefaults;
        params.application = fsref;
        params.argv = argvref;

        // log_debugf(0, STRING_CONST("Spawn process (LSOpenApplication):
        // %.*s"), STRING_FORMAT(localpath));
        ...
        if (status != 0) {
            ...
            log_errorf(
                ...
                STRING_CONST(
                    "Unable to spawn process for executable '%.*s': %.*s (%d)"),
        ...
        if (status == 0) {
            ...
            // Always "detached" with LSOpenApplication, not a child process at
            // all  Setup a kqueue to watch when process terminates so we can
            // emulate a wait
            proc->kq = kqueue();
            if (proc->kq < 0) {
                ...
                log_errorf(
                    0, ERROR_SYSTEM_CALL_FAIL,
                    STRING_CONST(
                        "Unable to create kqueue for process watch: %.*s (%d)"),
                    STRING_FORMAT(errmsg), proc->kq);
                proc->kq = 0;
            } else {
                struct kevent changes;
                EV_SET(&changes, (pid_t)pid, EVFILT_PROC, EV_ADD | EV_RECEIPT,
                ...
                if (ret != 1) {
                    ...
                    log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                               STRING_CONST("Unable to setup kqueue for "
                                            "process watch, failed to add "
                                            "event to kqueue: %.*s (%d)"),
    ...
    if (argc >= (sizeof(argv_stack) / sizeof(argv_stack[0])))
        argv = memory_allocate(0, sizeof(char *) * (argc + 1), 0,
                               MEMORY_PERSISTENT);
    ...
    if (pid == 0) {
        // Child
        if (proc->wd.length) {
            // log_debugf(0, STRING_CONST("Spawned child process, setting
            // working directory to %.*s"),
            //           STRING_FORMAT(proc->wd));
            environment_set_current_working_directory(STRING_ARGS(proc->wd));
        }

        // log_debugf(0, STRING_CONST("Child process executing: %.*s"),
        // STRING_FORMAT(proc->path));
        ...
        log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                   STRING_CONST(
                       "Child process failed execve() '%.*s': %.*s (%d) (%d)"),
    ...
    if (pid > 0) {
        ...
        /*if (proc->flags & PROCESS_DETACHED) {
                ...
                if (err > 0) {
                        ...
                        log_debugf(0, STRING_CONST("Child process returned:
        %d"), proc->code); return proc->code;
                }
        }*/
    } else {
        // Error
        ...
        log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                   STRING_CONST("Unable to spawn process '%.*s': %.*s (%d)"),
                   STRING_FORMAT(proc->path), STRING_FORMAT(errmsg),
                   proc->code);
...
stream_t *process_stderr(process_t *proc) {
...
int process_wait(process_t *proc) {
    ...
    while (GetExitCodeProcess(proc->hp, (LPDWORD)&proc->code)) {
        if ((proc->code != (int)STILL_ACTIVE) ||
            (proc->flags & PROCESS_DETACHED))
            break;
        thread_sleep(50);
        proc->code = -1;
    }
    ...
    if (proc->flags & PROCESS_MACOS_USE_OPENAPPLICATION) {
        if (proc->kq) {
            ...
            if (ret != 1) {
                ...
                string_const_t errmsg = system_error_message(err);
                log_warnf(0, WARNING_SYSTEM_CALL_FAIL,
                          STRING_CONST("Unable to wait on process, failed to "
                                       "read event from kqueue: %.*s (%d)"),
                          STRING_FORMAT(errmsg), err);
            }
            ...
            log_warn(0, WARNING_INVALID_VALUE,
                     STRING_CONST("Unable to wait on a process started with "
                                  "PROCESS_MACOS_USE_OPENAPPLICATION and no "
                                  "kqueue"));
    ...
    cstatus = 0;
    ret = waitpid(proc->pid, &cstatus,
                  (proc->flags & PROCESS_DETACHED) ? WNOHANG : 0);
    if (ret > 0) {
        ...
        string_const_t errmsg = system_error_message(err);
        log_warnf(0, WARNING_INVALID_VALUE,
                  STRING_CONST("waitpid(%d) failed: %.*s (%d) (returned %d)"),
