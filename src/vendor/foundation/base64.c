...
static const char _base64_decode[] = "|\0\0\0}rstuvwxyz{\0\0\0\0\0\0\0>?@"
                                     "ABCDEFGHIJKLMNOPQRSTUVW\0\0\0\0\0\0XYZ["
                                     "\\]^_`abcdefghijklmnopq";
static const char _base64_encode[] =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

size_t base64_encode(const void *source, size_t size, char *destination,
                     size_t capacity) {
    char *ptr;
...
size_t base64_decode(const char *source, size_t size, void *destination,
                     size_t capacity) {
    size_t i, blocksize;
    ...
    while (size && (cdst < cdstend)) {
        unsigned char in[4] = {
            0, 0, 0, 0
        }; //Always build blocks of 4 bytes to decode, pad with 0
        blocksize = 0;
        for (i = 0; size && (i < 4); i++) {
            char v = 0;
            while (size && !v) { // Consume one valid byte from input,
                // discarding invalid data
                v = *source++;
