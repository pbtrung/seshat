...
static uint32_t choise32(uint32_t x, uint32_t y, uint32_t z) {
    return z ^ (x & (y ^ z));
}
static uint32_t majority32(uint32_t x, uint32_t y, uint32_t z) {
    return ((x | y) & z) | (x & y);
}
static uint32_t rotate32(uint32_t x, uint32_t n) {
    return (x >> (n & 31)) | (x << (32 - (n & 31)));
}
static uint32_t shift32(uint32_t x, uint32_t n) {
    return x >> n;
}
...
static uint64_t choise64(uint64_t x, uint64_t y, uint64_t z) {
    return z ^ (x & (y ^ z));
}
static uint64_t majority64(uint64_t x, uint64_t y, uint64_t z) {
    return ((x | y) & z) | (x & y);
}
static uint64_t rotate64(uint64_t x, uint64_t n) {
    return (x >> (n & 63)) | (x << (64 - (n & 63)));
}
static uint64_t shift64(uint64_t x, uint64_t n) {
    return x >> n;
}
...
static void compress32(const uint32_t *wbox, uint32_t a, uint32_t b, uint32_t c,
...
static void compress64(const uint64_t *wbox, uint64_t a, uint64_t b, uint64_t c,
...
static void sha256_compress(sha256_t *digest, const unsigned char *buffer) {
    ...
    for (i = 16; i < 64; ++i)
        wbox[i] = gamma1_32(wbox[i - 2]) + wbox[i - 7] +
                  gamma0_32(wbox[i - 15]) + wbox[i - 16];

    for (i = 0; i < 64;) {
        compress32(wbox, sbox[0], sbox[1], sbox[2], sbox + 3, sbox[4], sbox[5],
                   sbox[6], sbox + 7, i++);

        compress32(wbox, sbox[7], sbox[0], sbox[1], sbox + 2, sbox[3], sbox[4],
                   sbox[5], sbox + 6, i++);

        compress32(wbox, sbox[6], sbox[7], sbox[0], sbox + 1, sbox[2], sbox[3],
                   sbox[4], sbox + 5, i++);

        compress32(wbox, sbox[5], sbox[6], sbox[7], sbox + 0, sbox[1], sbox[2],
                   sbox[3], sbox + 4, i++);

        compress32(wbox, sbox[4], sbox[5], sbox[6], sbox + 7, sbox[0], sbox[1],
                   sbox[2], sbox + 3, i++);

        compress32(wbox, sbox[3], sbox[4], sbox[5], sbox + 6, sbox[7], sbox[0],
                   sbox[1], sbox + 2, i++);

        compress32(wbox, sbox[2], sbox[3], sbox[4], sbox + 5, sbox[6], sbox[7],
                   sbox[0], sbox + 1, i++);

        compress32(wbox, sbox[1], sbox[2], sbox[3], sbox + 4, sbox[5], sbox[6],
                   sbox[7], sbox + 0, i++);
    }
...
static void sha512_compress(sha512_t *digest, const unsigned char *buffer) {
    ...
    for (i = 16; i < 80; ++i)
        wbox[i] = gamma1_64(wbox[i - 2]) + wbox[i - 7] +
                  gamma0_64(wbox[i - 15]) + wbox[i - 16];

    for (i = 0; i < 80;) {
        compress64(wbox, sbox[0], sbox[1], sbox[2], sbox + 3, sbox[4], sbox[5],
                   sbox[6], sbox + 7, i++);

        compress64(wbox, sbox[7], sbox[0], sbox[1], sbox + 2, sbox[3], sbox[4],
                   sbox[5], sbox + 6, i++);

        compress64(wbox, sbox[6], sbox[7], sbox[0], sbox + 1, sbox[2], sbox[3],
                   sbox[4], sbox + 5, i++);

        compress64(wbox, sbox[5], sbox[6], sbox[7], sbox + 0, sbox[1], sbox[2],
                   sbox[3], sbox + 4, i++);

        compress64(wbox, sbox[4], sbox[5], sbox[6], sbox + 7, sbox[0], sbox[1],
                   sbox[2], sbox + 3, i++);

        compress64(wbox, sbox[3], sbox[4], sbox[5], sbox + 6, sbox[7], sbox[0],
                   sbox[1], sbox + 2, i++);

        compress64(wbox, sbox[2], sbox[3], sbox[4], sbox + 5, sbox[6], sbox[7],
                   sbox[0], sbox + 1, i++);

        compress64(wbox, sbox[1], sbox[2], sbox[3], sbox + 4, sbox[5], sbox[6],
                   sbox[7], sbox + 0, i++);
    }
...
sha256_t *sha256_allocate(void) {
    sha256_t *digest =
        memory_allocate(0, sizeof(sha256_t), 0, MEMORY_PERSISTENT);
    sha256_initialize(digest);
    return digest;
}

void sha256_deallocate(sha256_t *digest) {
    sha256_finalize(digest);
    memory_deallocate(digest);
}

void sha256_initialize(sha256_t *digest) {
    digest->init = false;
    digest->current = 0;
    digest->length = 0;
...
void sha256_finalize(sha256_t *digest) {
    FOUNDATION_UNUSED(digest);
}

sha256_t *sha256_digest(sha256_t *digest, const void *buffer, size_t size) {
...
void sha256_digest_finalize(sha256_t *digest) {
...
uint256_t sha256_get_digest_raw(const sha256_t *digest) {
    uint256_t val = {
...
string_t sha256_get_digest(const sha256_t *digest, char *str, size_t length) {
...
sha512_t *sha512_allocate(void) {
    sha512_t *digest =
        memory_allocate(0, sizeof(sha512_t), 0, MEMORY_PERSISTENT);
    sha512_initialize(digest);
    return digest;
}

void sha512_deallocate(sha512_t *digest) {
    sha512_finalize(digest);
    memory_deallocate(digest);
}

void sha512_initialize(sha512_t *digest) {
    digest->init = false;
    digest->current = 0;
    digest->length = 0;
...
void sha512_finalize(sha512_t *digest) {
    FOUNDATION_UNUSED(digest);
}

sha512_t *sha512_digest(sha512_t *digest, const void *buffer, size_t size) {
    const size_t block_size = sizeof(digest->buffer);
...
void sha512_digest_finalize(sha512_t *digest) {
...
uint512_t sha512_get_digest_raw(const sha512_t *digest) {
    uint512_t val = {{
            digest->state[0], digest->state[1], digest->state[2],
            digest->state[3], digest->state[4], digest->state[5],
            digest->state[6], digest->state[7]
        }
    };
    return val;
}

string_t sha512_get_digest(const sha512_t *digest, char *str, size_t length) {
    uint512_t raw = sha512_get_digest_raw(digest);
    return string_from_uint512(str, length, raw);
}
