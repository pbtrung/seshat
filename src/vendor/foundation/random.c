...
// C implementation of the "Maximally equidistributed pseudorandom number
// generators via
// linear output transformations" from
// http://www.sciencedirect.com/science/article/pii/S0378475408002358  Put state
// array in thread-local storage for thread safety
...
static void _random_seed_buffer(unsigned int *buffer) {
    ...
    for (i = 0; i < RANDOM_STATE_SIZE; ++i)
        buffer[i] = ((tick_t)((uintptr_t)(buffer + i)) ^
                     (base + time_current() +
                      (i * RANDOM_HIGH_LIMIT * RANDOM_LOW_LIMIT))) &
                    0xFFFFFFFF;
}

static unsigned int *_random_allocate_buffer(void) {
    unsigned int *buffer =
        memory_allocate(0, sizeof(unsigned int) * (RANDOM_STATE_SIZE + 1), 0,
                        MEMORY_PERSISTENT);
...
static unsigned int *_random_thread_initialize(void) {
    unsigned int *buffer;
    ...
    if (!array_size(_random_available_state)) {
        ...
        buffer =
            _random_available_state[array_size(_random_available_state) - 1];
...
void random_thread_finalize(void) {
    if (!get_thread_state())
        return;
...
random_from_state(unsigned int *FOUNDATION_RESTRICT state) {
    ...
    if (state_index == 0) {
        ...
        return (
                   state[state_index] ^
                   (state[(state_index + RANDOM_HIGH_LIMIT) - RANDOM_STATE_SIZE + 1] &
                    RANDOM_BITMASK));
        ...
        bits1 =
            RANDOM_XOR_AND_LEFTSHIFT(24, state[state_index]) ^
            RANDOM_XOR_AND_RIGHTSHIFT(
                30, state[state_index + RANDOM_LOW_LIMIT - RANDOM_STATE_SIZE]);
        bits2 =
            RANDOM_XOR_AND_LEFTSHIFT(
                10,
                state[state_index + RANDOM_HIGH_LIMIT - RANDOM_STATE_SIZE]) ^
            (26U
             << state[(state_index + RANDOM_MID_LIMIT) - RANDOM_STATE_SIZE]);
        ...
        bits1 = RANDOM_XOR_AND_LEFTSHIFT(24, state[state_index]) ^
                RANDOM_XOR_AND_RIGHTSHIFT(
                    30, state[state_index + RANDOM_LOW_LIMIT]);
        bits2 =
            RANDOM_XOR_AND_LEFTSHIFT(
                10,
                state[state_index + RANDOM_HIGH_LIMIT - RANDOM_STATE_SIZE]) ^
    ...
    // else if( 2 <= state_index <= RANDOM_STATE_SIZE - RANDOM_HIGH_LIMIT - 1 )
    {
        ...
        bits1 = RANDOM_XOR_AND_LEFTSHIFT(24, state[state_index]) ^
                RANDOM_XOR_AND_RIGHTSHIFT(
                    30, state[state_index + RANDOM_LOW_LIMIT]);
        bits2 = RANDOM_XOR_AND_LEFTSHIFT(
                    10, state[state_index + RANDOM_HIGH_LIMIT]) ^
                (26U << state[state_index + RANDOM_MID_LIMIT]);
        state[state_index] = bits1 ^ bits2;
        state[state_index - 1] =
            bits0 ^ RANDOM_XOR_AND_RIGHTSHIFT(20, bits1) ^
...
real random_normalized(void) {
    ...
    const real result =
        (real)rng * (REAL_C(1.0) / REAL_C(18446744073709551616.0L));
#else
...
int32_t random32_gaussian_range(int32_t low, int32_t high) {
    const uint64_t cubic =
        ((((uint64_t)random32() + (uint64_t)random32()) +
          ((uint64_t)random32() + (uint64_t)random32()) + 2ULL) >>
         2ULL);
    if (low > high) {
        int32_t tmp = low;
        low = high;
...
real random_gaussian_range(real low, real high) {
    ...
    result =
        low +
        ((high - low) * REAL_C(0.33333333333333333333333333333) *
...
real random_triangle_range(real low, real high) {
    ...
    result = low + (high - low) * REAL_C(0.5) *
             (random_normalized() + random_normalized());
    if (result >= high)
        return math_real_dec(high, 1);
    return math_max(result, low);
}
