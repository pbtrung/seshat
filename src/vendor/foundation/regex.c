...
static regex_context_t _regex_execute_single(regex_t *regex, size_t op,
        const char *input, size_t inoffset,
        size_t inlength,
        string_const_t *captures,
        size_t maxcaptures);

static regex_context_t _regex_execute(regex_t *regex, size_t op,
                                      const char *input, size_t inoffset,
                                      size_t inlength, string_const_t *captures,
                                      size_t maxcaptures);

static FOUNDATION_FORCEINLINE FOUNDATION_CONSTCALL regex_context_t
_regex_context_nomatch(size_t next_op) {
    return (regex_context_t) {
        next_op, REGEXRES_NOMATCH
    };
}

static FOUNDATION_FORCEINLINE FOUNDATION_CONSTCALL regex_context_t
_regex_context_internal_failure(size_t next_op) {
    return (regex_context_t) {
        next_op, REGEXRES_INTERNAL_FAILURE
    };
}
...
static uint8_t _regex_parse_hex_digit(const char *p) {
    if ((*p >= '0') && (*p <= '9'))
        return (uint8_t)(*p - '0');
    if ((*p >= 'a') && (*p <= 'f'))
...
static uint8_t _regex_parse_hex(const char *p) {
    return ((_regex_parse_hex_digit(p) << 4) & 0xF0) |
           (_regex_parse_hex_digit(p + 1) & 0xF);
}

static size_t _regex_emit(regex_t **target, bool allow_grow, size_t ops, ...) {
    size_t iop;
    va_list arglist;
    ...
    if ((*target)->code_length + ops > (*target)->code_allocated) {
        ...
        *target =
            memory_reallocate(*target, sizeof(regex_t) + new_allocated, 0,
...
static size_t _regex_emit_buffer(regex_t **target, bool allow_grow, size_t ops,
...
static bool _regex_match_escape(char c, int code) {
    switch (code) {
    case REGEXCODE_NULL:
        return c == 0;
    case REGEXCODE_WHITESPACE:
        return string_find(STRING_CONST(STRING_WHITESPACE), c, 0) !=
               STRING_NPOS;
...
static size_t _regex_compile_quantifier(regex_t **target, bool allow_grow,
                                        size_t last_code_length,
                                        uint8_t quantifier) {
    size_t ret;
    size_t move_size;
    if (((*target)->code[last_code_length] == REGEXOP_EXACT_MATCH) &&
        ((*target)->code[last_code_length + 1] > 1)) {
        uint8_t last_char = (*target)->code[(*target)->code_length - 1];
        // The parser should break an REGEXOP_EXACT_MATCH block at a meta
        // character
        if (!FOUNDATION_VALIDATE((*target)->code[(*target)->code_length - 2] !=
                                 0))
            return REGEXERR_INTERNAL_FAILURE;
        --(*target)->code[last_code_length + 1];
        --(*target)->code_length;
        if ((ret = _regex_emit(target, allow_grow, 4, (int)quantifier,
                               REGEXOP_EXACT_MATCH, 1, (int)last_char)))
            return ret;
        return REGEXERR_OK;
    }
    ...
    if ((ret = _regex_emit(target, allow_grow, 1,
                           0))) // Make sure we have buffer space
        return ret;

    memmove((*target)->code + last_code_length + 1,
            (*target)->code + last_code_length, move_size);
    (*target)->code[last_code_length] = quantifier;

    return REGEXERR_OK;
}

static regex_context_t _regex_consume_longest(regex_t *regex, size_t op,
        const char *input,
        size_t inoffset, size_t inlength,
        string_const_t *captures,
        size_t maxcaptures) {
    ...
    // TODO: Optimization would be to stack all offsets from execute single,
    // then verify matching
    //      of remaining regex starting with highest saved offset
    /*lint -e{716} */
    while (true) {
        context = _regex_execute_single(regex, op, input, context.inoffset,
                                        inlength, 0, 0);
        ...
        next_context = _regex_execute(regex, context.op, input,
                                      context.inoffset, inlength, 0, 0);
        if (next_context.inoffset <= inlength) {
            if (captures)
                _regex_execute(regex, context.op, input, context.inoffset,
                               inlength, captures, maxcaptures);
            best_context = next_context;
        }
    }

    // Make sure we return the next op (_regex_execute_single returns next op
    // even on failure)
...
static regex_context_t _regex_consume_shortest(regex_t *regex, size_t op,
        const char *input,
        size_t inoffset, size_t inlength,
        string_const_t *captures,
        size_t maxcaptures) {
...
static size_t _regex_parse_group(regex_t **target, const char *pattern,
                                 size_t offset, size_t length,
                                 bool allow_grow) {
    // A single group cannot be more than 256 characters (size is stored in a
    // single byte)
    uint8_t buffer[256];
    size_t buffer_len = 0;
    ...
    while (!closed && !ret && (offset < length)) {
        if (pattern[offset] == ']') {
            ...
            if (buffer_len && (ret = _regex_emit_buffer(target, allow_grow,
                                     buffer_len, buffer)))
                break;
            offset++;
            closed = true;
...
static size_t _regex_parse(regex_t **target, const char *pattern, size_t offset,
                           size_t length, bool allow_grow, size_t level) {
    size_t ret = 0;
    ...
    while (offset < length) {
        ...
        case '(':
            ...
            if ((ret = _regex_emit(target, allow_grow, 2, REGEXOP_BEGIN_CAPTURE,
                                   (int)capture)))
                return ret;

            offset = _regex_parse(target, pattern, offset, length, allow_grow,
                                  level + 1);

            if ((offset > length) || (pattern[offset - 1] != ')'))
                return REGEXERR_MISMATCHED_CAPTURES;

            if ((ret = _regex_emit(target, allow_grow, 2, REGEXOP_END_CAPTURE,
                                   (int)capture)))
                return ret;
            break;
        ...
        case ')':
            if (branch_op != REGEXPARSE_NOBRANCH)
                (*target)->code[branch_op + 1] =
                    (uint8_t)((*target)->code_length - (branch_op + 2));
        ...
        case '[':
            last_code_length = (*target)->code_length;
            offset =
                _regex_parse_group(target, pattern, offset, length, allow_grow);
            if (offset > length)
                return ret;
            break;
        ...
        case '*':
        case '+':
            quantifier = ((uint8_t)pattern[offset - 1] == '*')
                         ? REGEXOP_ZERO_OR_MORE
                         : REGEXOP_ONE_OR_MORE;
            ...
            if ((offset < length) && (pattern[offset] == '?')) {
                quantifier = (quantifier == REGEXOP_ZERO_OR_MORE)
                             ? REGEXOP_ZERO_OR_MORE_SHORTEST
                             : REGEXOP_ONE_OR_MORE_SHORTEST;
                ++offset;
            }

            if ((ret = _regex_compile_quantifier(target, allow_grow,
                                                 last_code_length, quantifier)))
                return ret;
            break;
        ...
        case '\\':
            last_code_length = (*target)->code_length;
            if ((offset + 1 < length) && _regex_is_hex(pattern + offset)) {
                uint8_t val = _regex_parse_hex(pattern + offset);
                offset += 2;
                if ((ret = _regex_emit(target, allow_grow, 2,
                                       REGEXOP_META_MATCH, (int)val)))
                    return ret;
            } else if (offset < length) {
                code = _regex_encode_escape(pattern[offset++]);
                if (!code || (code > 0xFF)) {
                    /*lint -e{702} */
                    if ((ret = _regex_emit(target, allow_grow, 3,
                                           REGEXOP_META_MATCH, 0,
                                           (int)((code >> 8) & 0xFF))))
                        return ret;
                } else {
                    if ((ret = _regex_emit(target, allow_grow, 2,
                                           REGEXOP_META_MATCH, (int)code)))
                        return ret;
                }
            }
            break;
        ...
        case '|':
            ...
            if ((ret = _regex_emit(target, allow_grow, 4, 0, 0,
                                   REGEXOP_BRANCH_END, 0)))
                return ret;

            memmove((*target)->code + branch_begin + 2,
                    (*target)->code + branch_begin, size);
        ...
        default:
            ...
            while ((offset < length) &&
                   (string_find(STRING_CONST(REGEX_META_CHARACTERS),
                                pattern[offset], 0) == STRING_NPOS)) {
                ++size;
                ++offset;
            }

            last_code_length = (*target)->code_length;
            if ((ret = _regex_emit(target, allow_grow, 2, REGEXOP_EXACT_MATCH,
                                   size)))
                return ret;
            if ((ret = _regex_emit_buffer(target, allow_grow, size,
                                          (const uint8_t *)matchstart)))
                return ret;
            break;
        }
    }
...
// Returns a context with the next op offset to execute and the next input
// offset  to read (or error code if no match)
static regex_context_t _regex_execute_single(regex_t *regex, size_t op,
        const char *input, size_t inoffset,
        size_t inlength,
        string_const_t *captures,
    ...
    case REGEXOP_END_CAPTURE:
        ...
        if (captures && (capture < maxcaptures))
            captures[capture].length =
                (size_t)pointer_diff(input + inoffset, captures[capture].str);
        break;
    ...
    case REGEXOP_ANY_OF:
        ...
        for (ibuf = 0; ibuf < buffer_len; ++ibuf) {
            ...
            if (!cmatch) {
                if (_regex_match_escape(cin, regex->code[op + (++ibuf)] << 8))
                    break;
            } else if (cin == cmatch)
                break;
        }
    ...
    case REGEXOP_EXACT_MATCH:
        ...
        if ((matchlen > (inlength - inoffset)) ||
            !string_equal(input + inoffset, matchlen,
                          (const char *)regex->code + op, matchlen))
    ...
    case REGEXOP_META_MATCH:
        ...
        if (!cin) {
            if (_regex_match_escape(cmatch, regex->code[op++] << 8))
                break;
        } else if (cin == cmatch)
            break;
        return _regex_context_nomatch(op);
    ...
    case REGEXOP_ONE_OR_MORE:
        context = _regex_execute_single(regex, op, input, inoffset, inlength,
                                        captures, maxcaptures);
        if (context.inoffset > inlength)
            return context;
        context = _regex_consume_longest(regex, op, input, context.inoffset,
                                         inlength, captures, maxcaptures);
        inoffset = context.inoffset;
        op = context.op;
        break;
    ...
    case REGEXOP_ONE_OR_MORE_SHORTEST:
        ...
        if (context.inoffset > inlength)
            return context;
        context = _regex_consume_shortest(regex, op, input, context.inoffset,
                                          inlength, captures, maxcaptures);
        inoffset = context.inoffset;
        op = context.op;
        break;
    ...
    case REGEXOP_ZERO_OR_ONE:
        // Try matching with one, and if that succeeds verify complete match of
        // remaining data
        context = _regex_execute_single(regex, op, input, inoffset, inlength,
                                        captures, maxcaptures);
        if (context.inoffset <= inlength) {
        ...
        // Failed, try matching remainder with next op (now stored in context.op
        // by _regex_execute_single above)
        next_op = context.op;
        context = _regex_execute(regex, next_op, input, inoffset, inlength,
                                 captures, maxcaptures);
        if (context.inoffset <= inlength) {
            op = context.op;
            inoffset = context.inoffset;
    ...
    case REGEXOP_BRANCH:
        skip = regex->code[op++];
        context = _regex_execute(regex, op, input, inoffset, inlength, captures,
                                 maxcaptures);
        if (context.inoffset <= inlength) {
            inoffset = context.inoffset;
            op = context.op;
        } else {
            context = _regex_execute(regex, op + skip, input, inoffset,
                                     inlength, captures, maxcaptures);
            inoffset = context.inoffset;
            op = context.op;
        }
    ...
    default:
        log_errorf(0, ERROR_INTERNAL_FAILURE,
                   STRING_CONST("Regex encountered an unsupported op: %02x"),
...
static regex_context_t _regex_execute(regex_t *regex, size_t op,
    ...
    while (context.op < regex->code_length) {
        context =
            _regex_execute_single(regex, context.op, input, context.inoffset,
                                  inlength, captures, maxcaptures);
        if (context.inoffset > inlength)
            break;
    }
...
regex_t *regex_compile(const char *pattern, size_t pattern_length) {
    regex_t *compiled;
    compiled =
        memory_allocate(HASH_STRING, sizeof(regex_t) + pattern_length + 16, 0,
                        MEMORY_PERSISTENT);
    compiled->num_captures = 0;
    compiled->code_length = 0;
    compiled->code_allocated = pattern_length + 16;

    if (_regex_parse(&compiled, pattern, 0, pattern_length, true, 0) ==
        pattern_length)
        return compiled;
...
bool regex_parse(regex_t *regex, const char *pattern, size_t pattern_length) {
    regex_t *result = regex;
    return (_regex_parse(&result, pattern, 0, pattern_length, false, 0) ==
            pattern_length);
}

bool regex_match(regex_t *regex, const char *input, size_t inlength,
    ...
    if (regex->code[0] == REGEXOP_BEGINNING_OF_LINE) {
        regex_context_t context =
            _regex_execute(regex, 0, input, 0, inlength, captures, maxcaptures);
        if (context.inoffset <= inlength)
            return true;
    } else {
        for (iin = 0; iin < inlength; ++iin) {
            regex_context_t context = _regex_execute(
                                          regex, 0, input, iin, inlength, captures, maxcaptures);
            if (context.inoffset <= inlength)
                return true;
