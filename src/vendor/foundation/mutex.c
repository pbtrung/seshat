...
static void _mutex_initialize(mutex_t *mutex, const char *name, size_t length) {
    mutex->name =
        string_to_const(string_copy(mutex->name_buffer, 32, name, length));
...
mutex_t *mutex_allocate(const char *name, size_t length) {
    mutex_t *mutex = memory_allocate(
                         0, sizeof(mutex_t), 16, MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED);
...
bool mutex_lock(mutex_t *mutex) {
    ...
    if (pthread_mutex_lock(&mutex->mutex) != 0) {
        FOUNDATION_ASSERT_FAILFORMAT("unable to lock mutex %s",
                                     mutex->name.str);
        return false;
    }
#else
#error mutex_lock not implemented
    ...
    FOUNDATION_ASSERT_MSGFORMAT(
        !mutex->lockcount || (thread_id() == mutex->lockedthread),
        "Mutex lock acquired with lockcount > 0 (%d) and locked thread not "
        "self (%" PRIx64 " != %" PRIx64 ")",
...
bool mutex_unlock(mutex_t *mutex) {
    if (!mutex->lockcount) {
        log_warnf(0, WARNING_SUSPICIOUS,
                  STRING_CONST("Unable to unlock unlocked mutex %.*s"),
    ...
    if (pthread_mutex_unlock(&mutex->mutex) != 0) {
        FOUNDATION_ASSERT_FAILFORMAT("unable to unlock mutex %s",
                                     mutex->name.str);
        return false;
    }
#else
...
bool mutex_try_wait(mutex_t *mutex, unsigned int milliseconds) {
#if FOUNDATION_PLATFORM_WINDOWS
    DWORD ret;
#elif FOUNDATION_PLATFORM_POSIX || FOUNDATION_PLATFORM_PNACL
    struct timeval now;
    ...
    if (milliseconds == 0xFFFFFFFF) {
        ...
        if (ret == 0) {
            ...
            log_errorf(
                ...
                STRING_CONST("Unable to wait on mutex '%.*s': %.*s (%d)"),
        ...
        if (ret == 0) {
            ...
            log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                       STRING_CONST(
                           "Unable to wait (timed) on mutex '%.*s': %.*s (%d)"),
                       STRING_FORMAT(mutex->name), STRING_FORMAT(errmsg), ret);
        }
    }
...
void mutex_signal(mutex_t *mutex) {
    ...
    if (ret != 0) {
        string_const_t errmsg = system_error_message(ret);
        log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                   STRING_CONST("Unable to signal mutex '%.*s': %.*s (%d)"),
                   STRING_FORMAT(mutex->name), STRING_FORMAT(errmsg), ret);
    }
...
void *mutex_event_handle(mutex_t *mutex) {
    return mutex->event;
}
