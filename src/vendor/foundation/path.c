...
string_t path_clean(char *path, size_t length, size_t capacity) {
    ...
    for (ofs = 0; ofs < length;) {
        ...
        // Reduce multiple slashes "//./" unless directly after protocol
        // separator
        /*lint -e{850} */
        for (ahead = ofs; ahead < length; ++ahead) {
            if (path[ahead] == '.') { // Catch "/./" path segments
                if ((ahead + 1 < length) &&
                    ((path[ahead + 1] == '/') || (path[ahead + 1] == '\\'))) {
                    ++ahead;
                    continue;
                }
                break;
        ...
        ahead = ofs;
        num = 0;
        while ((ahead + 1 < length) && (path[ahead] == '.') &&
               (path[ahead + 1] == '.')) {
            if (ahead + 2 == length) {
                ++num;
                ahead = length;
        ...
        if (num > 0) {
            ...
            while ((back > reduce_limit) && (reduce < num)) {
                if (path[back - 1] == '.') {
                    // Only nuke starting "./" or "../" if we have an absolute
                    // protocol path
            ...
            if ((back == reduce_limit) && ((!back && (path[back] == '/')) ||
                                           (back && (path[back - 1] == '/')))) {
            ...
            if (reduce > 0) {
                ...
                length -= ahead - ofs;
                path[length] = 0;
                if ((length > 1) && clearsep && (length > reduce_limit) &&
                    (path[length - 1] == '/'))
                    path[--length] = 0;
                ofs = (ofs > (reduce_limit + 1))
                      ? ofs - 1
                      : (reduce_limit ? reduce_limit - 1 : 0);
                if (path[ofs] == ':')
                    ++ofs;
            }
        }
...
string_const_t path_base_file_name(const char *path, size_t length) {
    ...
    // For "dot" files, i.e files with names like "/path/to/.file", return the
    // dot name ".file"
    if (!end || (end == (start + 1)))
        end = STRING_NPOS;
    if (start != STRING_NPOS)
        return string_substr(path, length, start + 1,
                             (end != STRING_NPOS) ? (end - (start + 1))
                             : STRING_NPOS);
    return string_substr(path, length, 0, end);
}

string_const_t path_base_file_name_with_directory(const char *path,
        size_t length) {
    size_t start, end;
    if (!path || !length)
        return string_const(0, 0);
    ...
    // dot name ".file"
    if (!end || (end == (start + 1)) ||
        ((start != STRING_NPOS) && (end < start)))
        end = STRING_NPOS;
    return string_substr(path, length, 0,
                         (end != STRING_NPOS) ? end : STRING_NPOS);
}

string_const_t path_file_extension(const char *path, size_t length) {
    size_t start =
        string_find_last_of(path, length, STRING_CONST("/\\"), STRING_NPOS);
...
string_const_t path_subpath(const char *path, size_t length, const char *root,
                            size_t root_length) {
    ...
    if ((testpath.str == testpathstrip.str) ||
        (testroot.str == testrootstrip.str)) {
    ...
    if (!string_equal(testpath.str, testroot.length, testroot.str,
                      testroot.length))
        return string_empty();
    ...
    return string_substr(testpath.str, testpath.length, testroot.length,
                         STRING_NPOS);
}
...
string_const_t path_strip_protocol(const char *uri, size_t length) {
    size_t sepofs = string_find_string(uri, length, STRING_CONST("://"), 0);
    if (sepofs != STRING_NPOS) {
        bool has_drive_letter =
            (length > (sepofs + 4)) && (uri[sepofs + 4] == ':');
...
static string_t path_append_fragment(char *dst, size_t length, size_t capacity,
    ...
    return string_append(dst, length, capacity, path + offset,
                         pathlen - offset);
}

static string_t path_prepend_fragment(char *dst, size_t length, size_t capacity,
                                      const char *path, size_t pathlen) {
    size_t offset, num;
    bool lastsep, beginsep;
...
static string_t path_concat_impl(char *dest, size_t capacity, bool reallocate,
    ...
    if (reallocate) {
        ...
        if (totalsize >= capacity) {
            dest = dest ? memory_reallocate(dest, totalsize + 1, 0, capacity,
                                            MEMORY_NO_PRESERVE)
                   : memory_allocate(HASH_STRING, totalsize + 1, 0,
                                     MEMORY_PERSISTENT);
    ...
    do {
        ...
        if (ptr) {
            psize = va_arg(clist, size_t);
            if (psize)
                result = path_append_fragment(STRING_ARGS(result), capacity,
                                              ptr, psize);
        }
    } while (ptr);
    va_end(clist);
...
string_t path_concat(char *dest, size_t capacity, const char *first,
                     size_t first_length, const char *second,
                     size_t second_length) {
    return path_concat_varg(dest, capacity, first, first_length, second,
                            second_length, nullptr);
}

string_t path_concat_varg(char *dest, size_t capacity, const char *first,
                          size_t first_length, ...) {
    va_list list;
    string_t result;
...
string_t path_concat_vlist(char *dest, size_t capacity, const char *first,
                           size_t first_length, va_list list) {
    return path_concat_impl(dest, capacity, false, first, first_length, list);
}

string_t path_allocate_concat(const char *first, size_t first_length,
                              const char *second, size_t second_length) {
    return path_allocate_concat_varg(first, first_length, second, second_length,
                                     nullptr);
}

string_t path_allocate_concat_varg(const char *first, size_t first_length,
                                   ...) {
    va_list list;
    string_t result;
    va_start(list, first_length);
...
string_t path_allocate_concat_vlist(const char *first, size_t first_length,
                                    va_list list) {
    return path_concat_impl(0, 0, true, first, first_length, list);
}

string_t path_append(char *base, size_t base_length, size_t base_capacity,
                     const char *tail, size_t tail_length) {
    return path_append_fragment(base, base_length, base_capacity, tail,
                                tail_length);
}

string_t path_append_varg(char *base, size_t base_length, size_t base_capacity,
                          const char *tail, size_t tail_length, ...) {
    va_list list;
    string_t result;
    va_start(list, tail_length);
    result = path_append_vlist(base, base_length, base_capacity, tail,
                               tail_length, list);
    va_end(list);
    return result;
}

string_t path_append_vlist(char *base, size_t base_length, size_t base_capacity,
                           const char *tail, size_t tail_length, va_list list) {
    va_list clist;
    string_t result;
    void *ptr;
    size_t psize;
    ...
    result = path_append_fragment(STRING_ARGS(result), base_capacity, tail,
                                  tail_length);
    ...
    while (true) {
        ...
        psize = va_arg(clist, size_t);
        if (psize)
            result = path_append_fragment(STRING_ARGS(result), base_capacity,
                                          ptr, psize);
    }
    va_end(clist);
    return result;
...
string_t path_prepend(char *tail, size_t tail_length, size_t tail_capacity,
                      const char *base, size_t base_length) {
    return path_prepend_fragment(tail, tail_length, tail_capacity, base,
                                 base_length);
}

string_t path_prepend_varg(char *tail, size_t tail_length, size_t tail_capacity,
                           const char *base, size_t base_length, ...) {
    va_list list;
    string_t result;
    va_start(list, base_length);
    result = path_prepend_vlist(tail, tail_length, tail_capacity, base,
                                base_length, list);
    va_end(list);
    return result;
}

string_t path_prepend_vlist(char *tail, size_t tail_length,
    ...
    // TOSO: This should be done by putting fragments from back to front instead
    // of prepending (which is a memmove) each
    result = (string_t) {
        tail, tail_length
    };
    result = path_prepend_fragment(STRING_ARGS(result), tail_capacity, base,
                                   base_length);
    ...
    while (true) {
        ...
        psize = va_arg(clist, size_t);
        if (psize)
            result = path_prepend_fragment(STRING_ARGS(result), tail_capacity,
                                           ptr, psize);
    }
    va_end(clist);
    return result;
...
string_t path_absolute(char *path, size_t length, size_t capacity) {
    ...
    if (!path_is_absolute(path, length)) {
        ...
        abspath =
            path_prepend(STRING_ARGS(abspath), capacity, cwd.str, cwd.length);
    ...
    abspath = string_replace(STRING_ARGS(abspath), capacity,
                             STRING_CONST("/../"), STRING_CONST("/"), true);

    if (abspath.length >= 3 && (abspath.str[length - 3] == '/') &&
        (abspath.str[length - 2] == '.') && (abspath.str[length - 1] == '.')) {
        if (abspath.length == 3)
            abspath.length = 1;
        else
            abspath.length -= 3;
...
string_t path_allocate_absolute(const char *path, size_t length) {
...
string_t path_make_temporary(char *buffer, size_t capacity) {
    char uintbuffer[18];
    string_const_t tmpdir = environment_temporary_directory();
    string_t filename =
        string_from_uint(uintbuffer, 18, random64(), true, 0, '0');
    return path_concat(buffer, capacity, tmpdir.str, tmpdir.length,
