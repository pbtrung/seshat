/* stacktrace.c  -  Foundation library  -  Public Domain  -  2013 Mattias
...
typedef BOOL(WINAPI *EnumProcessesFn)(DWORD *lpidProcess, DWORD cb,
                                      DWORD *cbNeeded);
typedef BOOL(WINAPI *EnumProcessModulesFn)(HANDLE hProcess, HMODULE *lphModule,
        DWORD cb, LPDWORD lpcbNeeded);
typedef DWORD(WINAPI *GetModuleBaseNameFn)(HANDLE hProcess, HMODULE hModule,
        LPSTR lpBaseName, DWORD nSize);
typedef DWORD(WINAPI *GetModuleFileNameExFn)(HANDLE hProcess, HMODULE hModule,
...
typedef BOOL(WINAPI *SymInitializeFn)(HANDLE hProcess, PCSTR UserSearchPath,
                                      BOOL fInvadeProcess);
typedef DWORD(WINAPI *SymSetOptionsFn)(DWORD SymOptions);
typedef DWORD(WINAPI *SymGetOptionsFn)(VOID);
typedef DWORD64(WINAPI *SymLoadModule64Fn)(HANDLE hProcess, HANDLE hFile,
        PCSTR ImageName, PCSTR ModuleName,
...
typedef PVOID(WINAPI *SymFunctionTableAccess64Fn)(HANDLE hProcess,
        DWORD64 AddrBase);

typedef BOOL(WINAPI *StackWalk64Fn)(
...
typedef WORD(WINAPI *RtlCaptureStackBackTraceFn)(DWORD FramesToSkip,
...
static LONG WINAPI _stacktrace_exception_filter(LPEXCEPTION_POINTERS pointers) {
    FOUNDATION_UNUSED(pointers);
    log_error(0, ERROR_EXCEPTION,
              STRING_CONST("Exception occurred in stack trace!"));
    return EXCEPTION_EXECUTE_HANDLER;
}

#endif

static int _capture_stack_trace_helper(void **trace, size_t max_depth,
                                       size_t skip_frames, CONTEXT *context) {
...
#endif
    {
        ...
        while (succeeded && (current_depth < max_depth)) {
            succeeded = CallStackWalk64(
                            machine_type, process_handle, thread_handle, &stack_frame,
                            &context_copy, 0, CallSymFunctionTableAccess64,
                            CallSymGetModuleBase64, 0);
            if (succeeded) {
    ...
    __except (EXCEPTION_EXECUTE_HANDLER) {
        // We need to catch any exceptions within this function so they don't
        // get sent to the engine's error handler, hence causing an infinite
        // loop.
        return EXCEPTION_EXECUTE_HANDLER;
    }
#endif

    memset(trace + current_depth, 0,
           sizeof(void *) * (max_depth - current_depth));
...
static void _load_process_modules() {
    ...
    if (!CallEnumProcessModules(process_handle, module_handles,
                                sizeof(module_handles), &bytes))
        return;
    ...
    for (i = 0; i < module_count; ++i) {
        ...
        CallGetModuleInformation(process_handle, module_handle[i], &module_info,
                                 sizeof(module_info));
        CallGetModuleFileNameEx(process_handle, module_handle[i], image_name,
                                sizeof(image_name));
        CallGetModuleBaseName(process_handle, module_handle[i], module_name,
                              sizeof(module_name));

        GetFullPathNameA(image_name, sizeof(search_path), search_path,
                         &file_name);
        *file_name = 0;
        CallSymSetSearchPath(process_handle, search_path);

        CallSymLoadModule64(process_handle, module_handle[i], image_name,
                            module_name,
                            (uint64_t)((uintptr_t)module_info.lpBaseOfDll),
...
static void _load_process_modules(void) {
    ...
    if (_process_modules)
        memset(_process_modules, 0,
               sizeof(android_module_t) * _process_modules_size);

    stream_t *maps = fs_open_file(STRING_CONST("/proc/self/maps"), STREAM_IN);
    if (!maps) {
        log_error(0, ERROR_SYSTEM_CALL_FAIL,
                  STRING_CONST("Unable to read /proc/self/maps"));
        return;
    }

    while (!stream_eos(maps) &&
           (!_process_modules || (imod < _process_modules_size))) {
        ...
        uintptr_t end = (uintptr_t)string_to_uint64(
                            line.str + dashofs, line.length - dashofs, true);
        size_t modofs = (string_find_last_of(STRING_ARGS(line),
                                             STRING_CONST("/ "), STRING_NPOS) +
                         1);
        const char *module = line.str + modofs;

        if (!module[0] || (string_find_first_not_of(
                               module, line.length - modofs,
                               STRING_CONST("0123456789"), 0) == STRING_NPOS))
            continue;

        if (_process_modules && (imod > 0) &&
            /*( start == _process_modules[imod].address_end ) && */
            string_equal(module, line.length - modofs,
        ...
        if (_process_modules) {
            ...
            _process_modules[imod].name =
                string_copy(_process_modules[imod].namebuf,
                            sizeof(_process_modules[imod].namebuf), module,
                            line.length - modofs);
        }
        ++imod;
    }
    ...
    if (_process_modules && (imod == _process_modules_size))
        log_warn(0, WARNING_MEMORY,
                 STRING_CONST("Too many modules encountered"));

    if (!_process_modules) {
        _process_modules_size = imod + 16;
        _process_modules =
            memory_allocate(0, sizeof(android_module_t) * _process_modules_size,
                            0, MEMORY_PERSISTENT);
    }
}
...
static _Unwind_Reason_Code unwind_stack(struct _Unwind_Context *context,
                                        void *arg) {
...
static bool _initialize_stackwalker(void) {
...
#if FOUNDATION_PLATFORM_WINDOWS
    {
        ...
        CallStackWalk64 = (StackWalk64Fn)GetProcAddress(_stacktrace_dbghelp_dll,
                          "StackWalk64");
        if (!CallStackWalk64) {
        ...
        CallRtlCaptureStackBackTrace =
            (RtlCaptureStackBackTraceFn)GetProcAddress(
                _stacktrace_kernel_dll, "RtlCaptureStackBackTrace");
        if (!CallRtlCaptureStackBackTrace)
            CallRtlCaptureStackBackTrace =
                (RtlCaptureStackBackTraceFn)GetProcAddress(
                    _stacktrace_kernel_dll, "CaptureStackBackTrace");
        if (!CallRtlCaptureStackBackTrace) {
            log_warn(0, WARNING_SYSTEM_CALL_FAIL,
                     STRING_CONST(
                         "Unable to load get RtlCaptureStackBackTrace symbol"));
            return false;
        }
        _RtlCaptureContext = (RtlCaptureContextFn)GetProcAddress(
                                 _stacktrace_kernel_dll, "RtlCaptureContext");
    }
#endif
...
static void _finalize_stackwalker(void) {
#if FOUNDATION_PLATFORM_WINDOWS
    if (_stacktrace_kernel_dll)
...
size_t FOUNDATION_NOINLINE stacktrace_capture(void **trace, size_t max_depth,
        size_t skip_frames) {
    size_t num_frames = 0;
    ...
    ++skip_frames;
    if (CallRtlCaptureStackBackTrace) {
        num_frames = CallRtlCaptureStackBackTrace((DWORD)skip_frames,
                     (DWORD)max_depth, trace, 0);
        if (num_frames < max_depth)
            memset(trace + num_frames, 0,
                   sizeof(void *) * (max_depth - num_frames));
    ...
    android_trace_t stack_trace = {.trace = trace,
                                   .cur_depth = 0,
                                   .max_depth = max_depth,
...
static bool _initialize_symbol_resolve(void) {
...
#if FOUNDATION_PLATFORM_WINDOWS
    {
        ...
        CallEnumProcesses =
            (EnumProcessesFn)GetProcAddress(dll, "EnumProcesses");
        CallEnumProcessModules =
            (EnumProcessModulesFn)GetProcAddress(dll, "EnumProcessModules");
        CallGetModuleFileNameEx =
            (GetModuleFileNameExFn)GetProcAddress(dll, "GetModuleFileNameExA");
        CallGetModuleBaseName =
            (GetModuleBaseNameFn)GetProcAddress(dll, "GetModuleBaseNameA");
        CallGetModuleInformation =
            (GetModuleInformationFn)GetProcAddress(dll, "GetModuleInformation");
        ...
        CallSymInitialize =
            (SymInitializeFn)GetProcAddress(dll, "SymInitialize");
        CallSymSetOptions =
            (SymSetOptionsFn)GetProcAddress(dll, "SymSetOptions");
        CallSymGetOptions =
            (SymGetOptionsFn)GetProcAddress(dll, "SymGetOptions");
        CallSymLoadModule64 =
            (SymLoadModule64Fn)GetProcAddress(dll, "SymLoadModule64");
        CallSymSetSearchPath =
            (SymSetSearchPathFn)GetProcAddress(dll, "SymSetSearchPath");
        CallSymGetModuleInfo64 =
            (SymGetModuleInfo64Fn)GetProcAddress(dll, "SymGetModuleInfo64");
        CallSymGetLineFromAddr64 =
            (SymGetLineFromAddr64Fn)GetProcAddress(dll, "SymGetLineFromAddr64");
        CallSymGetSymFromAddr64 =
            (SymGetSymFromAddr64Fn)GetProcAddress(dll, "SymGetSymFromAddr64");
        CallSymGetModuleBase64 =
            (SymGetModuleBase64Fn)GetProcAddress(dll, "SymGetModuleBase64");
        CallSymFunctionTableAccess64 =
            (SymFunctionTableAccess64Fn)GetProcAddress(
                dll, "SymFunctionTableAccess64");

        if (!CallSymInitialize || !CallSymSetOptions || !CallSymGetOptions ||
            !CallSymLoadModule64 || !CallSymSetSearchPath ||
            !CallSymGetModuleInfo64 || !CallSymGetLineFromAddr64 ||
            !CallSymGetSymFromAddr64 || !CallSymGetModuleBase64 ||
            !CallSymFunctionTableAccess64)
            return _symbol_resolve_initialized;
...
static void _finalize_symbol_resolve(void) {
    ...
    FOUNDATION_PLATFORM_BSD
#define UNKNOWN_SYMBOL                                                         \
    "?"                                                                        \
    "?"
#endif

static FOUNDATION_NOINLINE string_t _resolve_stack_frames(char *buffer,
        size_t capacity,
        void *const *frames,
        size_t max_frames) {
#if FOUNDATION_PLATFORM_WINDOWS
    ...
    for (iaddr = 0; (iaddr < max_frames) && !last_was_main &&
         (resolved.length < capacity - 1);
         ++iaddr) {
        string_t line;
        ...
        // Allow first frame to be null in case of a function call to a null
        // pointer
        ...
        if (CallSymGetSymFromAddr64 &&
            CallSymGetSymFromAddr64(process_handle,
                                    (uint64_t)((uintptr_t)frames[iaddr]),
                                    &displacement64, symbol)) {
            size_t offset = 0;
            while ((offset < sizeof(symbol->Name)) &&
                   (symbol->Name[offset] < 32))
                ++offset;
        ...
        if (CallSymGetLineFromAddr64 &&
            CallSymGetLineFromAddr64(process_handle,
                                     (uint64_t)((uintptr_t)frames[iaddr]),
                                     &displacement, &line64)) {
        ...
        if (CallSymGetModuleInfo64 &&
            CallSymGetModuleInfo64(process_handle,
                                   (uint64_t)((uintptr_t)frames[iaddr]),
                                   &module64)) {
            size_t last_slash = STRING_NPOS;
            module_name = module64.ImageName;
            last_slash = string_rfind(module_name, string_length(module_name),
                                      '\\', STRING_NPOS);
        ...
        line = string_format(
                   resolved.str + resolved.length, capacity - resolved.length,
        ...
        if (string_equal(function_name, string_length(function_name),
                         STRING_CONST("main")))
            last_was_main = true;
    }
    ...
    // TODO: Use dladdr instead to avoid memory allocation with malloc in
    // backtrace_symbols
    string_t symbols = (string_t) {
        buffer, 0
    };
    ...
    for (unsigned int iframe = 0; iframe < max_frames; ++iframe) {
        ...
        if (length)
            symbols = string_append_varg(symbols.str, symbols.length, capacity,
                                         resolved[iframe], length,
    ...
    process_set_working_directory(
        proc, STRING_ARGS(environment_initial_working_directory()));
    ...
    resolved.length = 0;
    while (!stream_eos(procout) && (num_frames < requested_frames) &&
           !last_was_main) {
        line = string_format(
                   resolved.str + resolved.length, capacity - resolved.length,
        ...
        function =
            stream_read_line_buffer(procout, resolved.str + resolved.length,
                                    capacity - resolved.length, '\n');
        if (!function.length)
            function = string_copy(resolved.str + resolved.length,
                                   capacity - resolved.length,
        ...
        filename =
            stream_read_line_buffer(procout, resolved.str + resolved.length,
                                    capacity - resolved.length, '\n');
        if (!filename.length)
            filename = string_copy(resolved.str + resolved.length,
                                   capacity - resolved.length,
                                   STRING_CONST(UNKNOWN_SYMBOL));
    ...
    for (unsigned int iaddr = 0; iaddr < max_frames; ++iaddr) {
        ...
        line = string_format(
                   ...
                   (uintptr_t)frames[iaddr], (uintptr_t)relativeframe,
                   STRING_FORMAT(module));
        symbols =
            string_append(STRING_ARGS(symbols), capacity, STRING_ARGS(line));
    }
...
string_t stacktrace_resolve(char *str, size_t length, void *const *trace,
                            size_t max_depth, size_t skip_frames) {
