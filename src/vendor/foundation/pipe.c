...
stream_t *pipe_allocate(void) {
    stream_pipe_t *pipestream = memory_allocate(
                                    HASH_STREAM, sizeof(stream_pipe_t), 8, MEMORY_PERSISTENT);
...
void pipe_initialize(stream_pipe_t *pipestream) {
    ...
    pipestream->path = string_allocate_format(
                           STRING_CONST("pipe://0x%" PRIfixPTR), (uintptr_t)pipestream);
...
#if FOUNDATION_PLATFORM_WINDOWS
    {
        ...
        if (!CreatePipe(&hread, &hwrite, &security_attribs, 0)) {
            ...
            log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                       STRING_CONST("Unable to create unnamed pipe: %.*s"),
                       STRING_FORMAT(errmsg));
        } else {
            pipestream->fd_read =
                _open_osfhandle((intptr_t)hread, _O_RDONLY | _O_BINARY);
            pipestream->fd_write =
                _open_osfhandle((intptr_t)hwrite, _O_WRONLY | _O_BINARY);
            if (!pipestream->fd_read || !pipestream->fd_write) {
                string_const_t errmsg = system_error_message(0);
                log_errorf(
                    0, ERROR_SYSTEM_CALL_FAIL,
                    STRING_CONST(
                        "Unable to create unnamed pipe file descriptors: %.*s"),
                    STRING_FORMAT(errmsg));
                ...
                if (!SetNamedPipeHandleState(hread, &mode, 0, 0)) {
                    ...
                    log_errorf(
                        0, ERROR_SYSTEM_CALL_FAIL,
                        STRING_CONST(
                            "Unable to create unnamed pipe handle state: %.*s"),
                        STRING_FORMAT(errmsg));
                }
            }
        }
...
void *pipe_read_handle(stream_t *stream) {
    stream_pipe_t *pipestream = (stream_pipe_t *)stream;
    intptr_t handle = stream && (stream->type == STREAMTYPE_PIPE)
                      ? _get_osfhandle(pipestream->fd_read)
                      : 0;
    return (void *)handle;
}
...
void *pipe_write_handle(stream_t *stream) {
    stream_pipe_t *pipestream = (stream_pipe_t *)stream;
    intptr_t handle = stream && (stream->type == STREAMTYPE_PIPE)
                      ? _get_osfhandle(pipestream->fd_write)
                      : 0;
    return (void *)handle;
}
...
int pipe_read_fd(stream_t *stream) {
    stream_pipe_t *pipestream = (stream_pipe_t *)stream;
    return stream && (stream->type == STREAMTYPE_PIPE) ? pipestream->fd_read
           : 0;
}

int pipe_write_fd(stream_t *stream) {
    stream_pipe_t *pipestream = (stream_pipe_t *)stream;
    return stream && (stream->type == STREAMTYPE_PIPE) ? pipestream->fd_write
           : 0;
}
...
static size_t _pipe_stream_read(stream_t *stream, void *dest, size_t num) {
    ...
    if (pipestream->fd_read && ((pipestream->mode & STREAM_IN) != 0)) {
        ...
        do {
            ssize_t num_read =
                read(pipestream->fd_read, pointer_offset(dest, total_read),
...
static size_t _pipe_stream_write(stream_t *stream, const void *source,
                                 size_t num) {
    stream_pipe_t *pipestream = (stream_pipe_t *)stream;
    FOUNDATION_ASSERT(stream->type == STREAMTYPE_PIPE);
    if (pipestream->fd_write && ((pipestream->mode & STREAM_OUT) != 0)) {
        ...
        do {
            ssize_t num_written =
                write(pipestream->fd_write,
                      pointer_offset_const(source, total_written),
...
static void _pipe_stream_seek(stream_t *stream, ssize_t offset,
