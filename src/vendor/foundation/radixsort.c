...
static bool radixsort_create_histograms(radixsort_t *sort,
    ...
    // Don't allow temporal coherence if increasing in size as it might
...
static const radixsort_index_t *
radixsort_int(radixsort_t *sort, const void *input, radixsort_index_t num) {
    const radixsort_data_t data_type = sort->type;
...
static const radixsort_index_t *
radixsort_float(radixsort_t *sort, const void *input, radixsort_index_t num) {
    const radixsort_data_t data_type = sort->type;
    const unsigned int data_size = _radixsort_data_size[data_type];
    const unsigned int data_shift = _radixsort_data_shift[data_type];
    ...
    // Number of negatives is the last 128 values in the MSB histogram
    //(last, since we deal with system byte ordering in
    //radixsort_create_histograms)
            ...
            if (count[*input_bytes] != num) {
                ...
                *offset++ = 0;
...

                {
                    ...
                    while (indices != indices_end) {
                        radixsort_index_t id = *indices++;
                        indices_next[offset[input_bytes[id << data_shift]]++] =
                            id;
                    }
                }
                ...
                *offset-- = 0;
                ...
                if (data_type == RADIXSORT_FLOAT32) {
                    ...
                    for (ival = 0; ival < num; ++ival) {
                        unsigned int radix =
                            input_uint[sort->indices[0][ival]] >> 24;
                        if (radix < 128)
                            sort->indices[1][sort->offset[radix]++] =
                                sort->indices[0][ival]; // Positive
                        else
                            sort->indices[1][--sort->offset[radix]] =
                                sort->indices[0]
                                [ival]; // Negative, reverse order
                    }
                } else { // if( data_type == RADIXSORT_FLOAT64 )
                    const uint64_t *input_uint = input;
                    for (ival = 0; ival < num; ++ival) {
                        unsigned int radix =
                            (unsigned int)(input_uint[sort->indices[0][ival]] >>
                                           56ULL);
                        if (radix < 128)
                            sort->indices[1][sort->offset[radix]++] =
                                sort->indices[0][ival]; // Positive
...
const radixsort_index_t *radixsort_sort(radixsort_t *sort, const void *input,
...
radixsort_t *radixsort_allocate(radixsort_data_t type, radixsort_index_t num) {
    radixsort_t *sort;
    sort = memory_allocate(
               0,
               sizeof(radixsort_t) +
               /* 2 index tables */ (2 * sizeof(radixsort_index_t) * num) +
               /* histograms */
               (256 * _radixsort_data_size[type] * sizeof(radixsort_index_t)) +
               /* offset table */ (256 * sizeof(radixsort_index_t)),
               0, MEMORY_PERSISTENT);
    sort->indices[0] = pointer_offset(sort, sizeof(radixsort_t));
    sort->indices[1] =
        pointer_offset(sort->indices[0], sizeof(radixsort_index_t) * num);
    sort->histogram =
        pointer_offset(sort->indices[1], sizeof(radixsort_index_t) * num);
    sort->offset =
        pointer_offset(sort->histogram, sizeof(radixsort_index_t) * 256 *
                       _radixsort_data_size[type]);
...
void radixsort_initialize(radixsort_t *sort, radixsort_data_t type,
                          radixsort_index_t num) {
    radixsort_index_t i;
