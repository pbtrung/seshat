...
static DWORD WINAPI GetCurrentProcessorNumberFallback(VOID) {
    return 0;
}

static GetCurrentProcessorNumberFn _fnGetCurrentProcessorNumber =
    GetCurrentProcessorNumberFallback;
...
int _thread_initialize(void) {
    ...
    getprocidfn = (GetCurrentProcessorNumberFn)GetProcAddress(
                      kernel32, "GetCurrentProcessorNumber");
    if (getprocidfn)
        _fnGetCurrentProcessorNumber = getprocidfn;

    setthreaddescfn = (SetThreadDescriptionFn)GetProcAddress(
...
static int _thread_try(void *data) {
...
_set_debugger_thread_name(const char *threadname) {
    ...
    prev_filter =
        SetUnhandledExceptionFilter(_thread_set_name_exception_filter);
#else
    __try
#endif
    {
...
#else
    __except (EXCEPTION_CONTINUE_EXECUTION) { // Does EXCEPTION_EXECUTE_HANDLER
        // require a debugger present?
...
void thread_set_name(const char *name, size_t length) {
    ...
    if (_fnSetThreadDescriptionFn) {
        wchar_t wname[64];
        wstring_from_string(wname, sizeof(wname) / sizeof(wname[0]), name,
                            length);
    ...
    if (self) {
        string_t newname = string_copy(self->namebuffer,
                                       sizeof(self->namebuffer), name, length);
...
thread_t *thread_allocate(thread_fn fn, void *data, const char *name,
                          size_t length, thread_priority_t priority,
                          unsigned int stacksize) {
    thread_t *thread = memory_allocate(
                           0, sizeof(thread_t), 0, MEMORY_PERSISTENT | MEMORY_ZERO_INITIALIZED);
    thread_initialize(thread, fn, data, name, length, priority, stacksize);
    return thread;
}

void thread_initialize(thread_t *thread, thread_fn fn, void *data,
    ...
    newname = string_copy(thread->namebuffer, sizeof(thread->namebuffer), name,
                          length);
    thread->arg = data;
...
void thread_finalize(thread_t *thread) {
    thread_join(thread);
...
bool thread_start(thread_t *thread) {
    ...
    thread->handle = _beginthreadex(nullptr, thread->stacksize, _thread_entry,
                                    thread, 0, nullptr);
    if (!thread->handle) {
        ...
        log_errorf(
            ...
            STRING_CONST(
                "Unable to create thread: CreateThread failed: %.*s (%d)"),
    ...
    if (err) {
        ...
        log_errorf(
            0, ERROR_OUT_OF_MEMORY,
            STRING_CONST(
                "Unable to create thread: pthread_create failed: %.*s (%d)"),
            STRING_FORMAT(errmsg), err);
        return false;
    }
...
bool thread_is_started(const thread_t *thread) {
    return thread ? atomic_load32(&thread->state, memory_order_acquire) >= 1
           : false;
}
...
bool thread_try_wait(unsigned int milliseconds) {
    thread_t *thread = get_thread_self();
    return thread ? (beacon_try_wait(&thread->beacon, milliseconds) >= 0)
           : false;
}
...
void *thread_attach_jvm(void) {
    ...
    // TODO: According to the native activity, the java env can only be used in
    // the main thread (calling ANativeActivityCallbacks)
    jint result =
        (*app->activity->vm)
        ->AttachCurrentThread(app->activity->vm,
                              (const struct JNINativeInterface ** *)&env,
                              &attach_args);
    if (result < 0)
        log_warnf(0, WARNING_SYSTEM_CALL_FAIL,
                  STRING_CONST("Unable to attach thread to Java VM (%d)"),
                  result);
