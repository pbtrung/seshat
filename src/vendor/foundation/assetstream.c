/* assetstream.c  -  Foundation library  -  Public Domain  -  2013 Mattias
...
static size_t asset_stream_write(stream_t *stream, const void *source,
                                 size_t num) {
...
static bool asset_stream_eos(stream_t *stream) {
    ...
    return !asset || !asset->asset ||
           ((off_t)asset->position >= AAsset_getLength(asset->asset));
}
...
static size_t asset_stream_size(stream_t *stream) {
    stream_asset_t *asset = (stream_asset_t *)stream;
    ssize_t length =
        (asset && asset->asset ? AAsset_getLength(asset->asset) : 0);
    return (length > 0) ? (size_t)length : 0;
}

static void asset_stream_seek(stream_t *stream, ssize_t offset,
...
static stream_t *asset_stream_clone(stream_t *stream) {
    return stream ? asset_stream_open(STRING_ARGS(stream->path), stream->mode)
           : 0;
}

stream_t *asset_stream_open(const char *path, size_t length,
                            unsigned int mode) {
    string_t finalpath;
    char *pathptr;
    ...
    if ((length >= 6) && string_equal(path, 6, STRING_CONST("asset:"))) {
        ...
        finalpath = string_concat(buffer, sizeof(buffer), "asset://",
                                  path[0] == '/' ? 7 : 8, path, length);
        pathptr = finalpath.str + 8;
    }
    if (*pathptr == '/')
        ++path;

    AAsset *assetobj = AAssetManager_open(android_app()->activity->assetManager,
                                          pathptr, AASSET_MODE_RANDOM);
    if (!assetobj)
        return 0;

    stream_asset_t *asset =
        memory_allocate(HASH_STREAM, sizeof(stream_asset_t), 8,
