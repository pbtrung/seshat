...
static void _bitbuffer_get(bitbuffer_t *FOUNDATION_RESTRICT bitbuffer) {
    if (bitbuffer->buffer < bitbuffer->end) {
        // For alignment required archs, we know it is 32-bit aligned already so
        // safe casts below
        void *bufferptr = bitbuffer->buffer;
        bitbuffer->pending_read = bitbuffer->swap
                                  ? byteorder_swap32(*(uint32_t *)bufferptr)
                                  : *(uint32_t *)bufferptr;
        bitbuffer->buffer += 4;
...
static void _bitbuffer_put(bitbuffer_t *FOUNDATION_RESTRICT bitbuffer) {
    if (bitbuffer->buffer < bitbuffer->end) {
        ...
        void *bufferptr = bitbuffer->buffer;
        *(uint32_t *)bufferptr =
            bitbuffer->swap ? byteorder_swap32(bitbuffer->pending_write)
            : bitbuffer->pending_write;
        bitbuffer->buffer += 4;
    } else if (bitbuffer->stream) {
...
bitbuffer_t *bitbuffer_allocate_buffer(void *buffer, size_t size, bool swap) {
    bitbuffer_t *bitbuffer =
        memory_allocate(0, sizeof(bitbuffer_t), 0, MEMORY_PERSISTENT);
...
void bitbuffer_initialize_buffer(bitbuffer_t *bitbuffer, void *buffer,
                                 size_t size, bool swap) {
...
uint128_t bitbuffer_read128(bitbuffer_t *bitbuffer, unsigned int bits) {
    if (bits <= 64) {
        ...
        const uint128_t value = {.word[0] = bitbuffer_read64(bitbuffer, bits),
                                 .word[1] = 0
                                };
#else
        uint128_t value;
...
uint32_t bitbuffer_read32(bitbuffer_t *bitbuffer, unsigned int bits) {
    ...
    ret = (curbits == 32)
          ? bitbuffer->pending_read
          : ((bitbuffer->pending_read >> bitbuffer->offset_read) &
             ((1U << curbits) - 1));
    ...
    ret |= (bitbuffer->pending_read & ((1U << (bits - curbits)) - 1))
           << curbits;
...
void bitbuffer_write128(bitbuffer_t *bitbuffer, uint128_t value,
                        unsigned int bits) {
    if (bits <= 64) {
...
void bitbuffer_write64(bitbuffer_t *bitbuffer, uint64_t value,
                       unsigned int bits) {
    if (bits <= 32) {
...
void bitbuffer_write32(bitbuffer_t *bitbuffer, uint32_t value,
                       unsigned int bits) {
    unsigned int curbits;
    ...
    bitbuffer->pending_write |=
        ((curbits == 32) ? value : (value & ((1U << curbits) - 1)))
        << bitbuffer->offset_write;
    ...
    bitbuffer->pending_write =
        (value >> curbits) & ((1U << (bits - curbits)) - 1);
