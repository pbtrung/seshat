...
error_handler_fn error_handler(void) {
    return get_thread_error_handler();
}

void error_set_handler(error_handler_fn handler) {
...
void _error_context_push(const char *name, size_t name_length, const char *data,
                         size_t data_length) {
    error_context_t *context = get_thread_error_context();
    if (!context) {
        size_t capacity =
            sizeof(error_context_t) +
            (sizeof(error_frame_t) * foundation_config().error_context_depth);
        context = memory_allocate(0, capacity, 0,
...
void _error_context_thread_finalize(void) {
    error_context_t *context = get_thread_error_context();
    if (context) {
        FOUNDATION_ASSERT_MSG(
            !context->depth,
            "Error context thread exit with non-zero context stack");
