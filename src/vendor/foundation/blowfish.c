/* blowfish.c  -  Foundation library  -  Public Domain  -  2013 Mattias Jansson
 * / Rampant Pixels
 *
...
static const uint32_t _blowfish_parray_init[BLOWFISH_SUBKEYS] = {
    ...
    0xB5470917U, 0x9216D5D9U, 0x8979FB1BL
};

static const uint32_t
_blowfish_sboxes_init[BLOWFISH_SBOXES][BLOWFISH_SBOXENTRIES] = {
...
#define FEISTEL(val)                                                           \
    (((blowfish->sboxes[0][(val >> 24) & 0xFF] +                               \
       blowfish->sboxes[1][(val >> 16) & 0xFF]) ^                              \
      blowfish->sboxes[2][(val >> 8) & 0xFF]) +                                \
...
void _blowfish_encrypt_words(const blowfish_t *blowfish, uint32_t *lvalres,
    ...
    lval ^= blowfish->parray[0];
    hval ^= FEISTEL(lval) ^ blowfish->parray[1];
    lval ^= FEISTEL(hval) ^ blowfish->parray[2];
    hval ^= FEISTEL(lval) ^ blowfish->parray[3];
    lval ^= FEISTEL(hval) ^ blowfish->parray[4];
    hval ^= FEISTEL(lval) ^ blowfish->parray[5];
    lval ^= FEISTEL(hval) ^ blowfish->parray[6];
    hval ^= FEISTEL(lval) ^ blowfish->parray[7];
    lval ^= FEISTEL(hval) ^ blowfish->parray[8];
    hval ^= FEISTEL(lval) ^ blowfish->parray[9];
    lval ^= FEISTEL(hval) ^ blowfish->parray[10];
    hval ^= FEISTEL(lval) ^ blowfish->parray[11];
    lval ^= FEISTEL(hval) ^ blowfish->parray[12];
    hval ^= FEISTEL(lval) ^ blowfish->parray[13];
    lval ^= FEISTEL(hval) ^ blowfish->parray[14];
    hval ^= FEISTEL(lval) ^ blowfish->parray[15];
    lval ^= FEISTEL(hval) ^ blowfish->parray[16];
    hval ^= blowfish->parray[17];
...
void _blowfish_decrypt_words(const blowfish_t *blowfish, uint32_t *lvalres,
    ...
    lval ^= blowfish->parray[17];
    hval ^= FEISTEL(lval) ^ blowfish->parray[16];
    lval ^= FEISTEL(hval) ^ blowfish->parray[15];
    hval ^= FEISTEL(lval) ^ blowfish->parray[14];
    lval ^= FEISTEL(hval) ^ blowfish->parray[13];
    hval ^= FEISTEL(lval) ^ blowfish->parray[12];
    lval ^= FEISTEL(hval) ^ blowfish->parray[11];
    hval ^= FEISTEL(lval) ^ blowfish->parray[10];
    lval ^= FEISTEL(hval) ^ blowfish->parray[9];
    hval ^= FEISTEL(lval) ^ blowfish->parray[8];
    lval ^= FEISTEL(hval) ^ blowfish->parray[7];
    hval ^= FEISTEL(lval) ^ blowfish->parray[6];
    lval ^= FEISTEL(hval) ^ blowfish->parray[5];
    hval ^= FEISTEL(lval) ^ blowfish->parray[4];
    lval ^= FEISTEL(hval) ^ blowfish->parray[3];
    hval ^= FEISTEL(lval) ^ blowfish->parray[2];
    lval ^= FEISTEL(hval) ^ blowfish->parray[1];
    hval ^= blowfish->parray[0];
...
void blowfish_initialize(blowfish_t *blowfish, const void *key, size_t length) {
    size_t capped_length =
        (length <= BLOWFISH_MAXKEY) ? length : BLOWFISH_MAXKEY;
    ...
    memcpy(blowfish->sboxes, _blowfish_sboxes_init,
           4 * BLOWFISH_SBOXES * BLOWFISH_SBOXENTRIES);
