/* ringbuffer.c  -  Foundation library  -  Public Domain  -  2013 Mattias
...
ringbuffer_t *ringbuffer_allocate(size_t size) {
    ringbuffer_t *buffer =
        memory_allocate(0, sizeof(ringbuffer_t) + size, 0, MEMORY_PERSISTENT);
...
size_t ringbuffer_read(ringbuffer_t *buffer, void *dest, size_t num) {
    ...
    if ((do_read < num) && (offset_read == 0) && (offset_write > 0))
        do_read += ringbuffer_read(
                       buffer, dest ? pointer_offset(dest, do_read) : dest, num - do_read);
...
size_t ringbuffer_write(ringbuffer_t *buffer, const void *source, size_t num) {
    ...
    if (offset_write >= offset_read) {
        ...
        // Don't read so write aligns to read, then the entire buffer is
        // discarded
    ...
    if (offset_write == buffer_size) {
        FOUNDATION_ASSERT_MSG(
            buffer->offset_read,
            "Ring buffer internal failure, discarded entire buffer");
        offset_write = 0;
    }
    ...
    if ((do_write < num) && (offset_write == 0) && (offset_read > 0))
        do_write += ringbuffer_write(
                        buffer, pointer_offset_const(source, do_write), num - do_write);
...
static size_t _ringbuffer_stream_read(stream_t *stream, void *dest,
                                      size_t num) {
    ...
    while (num_read < num) {
        ...
        num_read += ringbuffer_read(
                        buffer, dest ? pointer_offset(dest, num_read) : 0, num - num_read);
    }
...
static size_t _ringbuffer_stream_write(stream_t *stream, const void *source,
                                       size_t num) {
    stream_ringbuffer_t *rbstream = (stream_ringbuffer_t *)stream;
    ringbuffer_t *buffer = RINGBUFFER_FROM_STREAM(rbstream);
    ...
    while (num_write < num) {
        ...
        num_write += ringbuffer_write(
                         buffer, source ? pointer_offset_const(source, num_write) : 0,
                         num - num_write);
    }
...
static bool _ringbuffer_stream_eos(stream_t *stream) {
    ...
    return buffer->total_size ? (buffer->total_read >= buffer->total_size)
           : false;
}
...
static void _ringbuffer_stream_seek(stream_t *stream, ssize_t offset,
                                    stream_seek_mode_t direction) {
    if ((direction != STREAM_SEEK_CURRENT) || (offset < 0)) {
        log_error(0, ERROR_UNSUPPORTED,
                  STRING_CONST("Invalid call, only forward seeking allowed on "
                               "ringbuffer streams"));
        return;
    }
...
stream_t *ringbuffer_stream_allocate(size_t buffer_size, size_t total_size) {
    stream_ringbuffer_t *bufferstream = memory_allocate(
                                            0, sizeof(stream_ringbuffer_t) + buffer_size, 0, MEMORY_PERSISTENT);
...
void ringbuffer_stream_initialize(stream_ringbuffer_t *stream,
                                  size_t buffer_size, size_t total_size) {
    ...
    stream->sequential = 1;
    stream->path = string_allocate_format(
                       STRING_CONST("ringbuffer://0x%" PRIfixPTR), (uintptr_t)stream);
...
void _ringbuffer_stream_initialize(void) {
    ...
    _ringbuffer_stream_vtable.available_read =
        _ringbuffer_stream_available_read;
