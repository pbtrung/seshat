...
static char _log_warning_name[LOG_WARNING_NAMES][18] = {
    "performance", "deprecated",       "invalid value", "memory", "unsupported",
    "suspicious",  "system call fail", "deadlock",      "script", "resource"
};

#define LOG_ERROR_NAMES 16
static char _log_error_name[LOG_ERROR_NAMES][18] = {"none",
                                                    "invalid value",
                                                    "unsupported",
                                                    "not implemented",
                                                    "out of memory",
...
_log_outputf(hash_t context, error_level_t severity, const char *prefix,
             size_t prefix_length, const char *format, size_t format_length,
             va_list list, void *std) {
    ...
    /*lint -e716 */
    while (1) {
        // This is guaranteed to always fit in minimum size of 383 bytes defined
        // above, so need is always > 0
        if (_log_prefix)
            need = snprintf(buffer, (unsigned int)size,
                            "[%d:%02d:%02d.%03d] <%" PRIx64 ":%u> %.*s",
                            timestamp.hours, timestamp.minutes,
                            timestamp.seconds, timestamp.milliseconds, tid, pid,
                            (int)prefix_length, prefix);
        else
            need = snprintf(buffer, (unsigned int)size, "%.*s",
                            (int)prefix_length, prefix);
        ...
        remain = size - need;
        {
            ...
            va_copy(clist, list);
            more =
                vsnprintf(buffer + need, (unsigned int)remain, format, clist);
            va_end(clist);
        }
        ...
        if ((more > -1) && (more < remain)) {
            ...
            if (_log_stdout)
                __android_log_write(ANDROID_LOG_DEBUG + severity - 1,
                                    environment_application()->short_name.str,
                                    buffer);
#elif FOUNDATION_PLATFORM_TIZEN
            FOUNDATION_UNUSED(std);
            if (_log_stdout)
                dlog_print(DLOG_DEBUG + severity - 1,
                           environment_application()->short_name.str, "%s",
                           buffer);
            ...
            if (_log_handler)
                _log_handler(context, severity, buffer,
                             (unsigned int)(endl - 1));
        ...
        buffer =
            memory_allocate(0, (unsigned int)(size + 2), 0, MEMORY_TEMPORARY);
    }
    if (buffer != local_buffer)
        memory_deallocate(buffer);
}
...
void log_debugf(hash_t context, const char *format, size_t length, ...) {
    ...
    if (log_suppress(context) < ERRORLEVEL_DEBUG)
        _log_outputf(context, ERRORLEVEL_DEBUG, "", 0, format, length, list,
                     stdout);
    va_end(list);
}
...
void log_infof(hash_t context, const char *format, size_t length, ...) {
    ...
    if (log_suppress(context) < ERRORLEVEL_INFO)
        _log_outputf(context, ERRORLEVEL_INFO, "", 0, format, length, list,
                     stdout);
    va_end(list);
}
...
void log_warnf(hash_t context, warning_t warn, const char *format,
    ...
    if (warn < LOG_WARNING_NAMES)
        prefix = string_format(buffer, sizeof(buffer),
                               STRING_CONST("WARNING [%s]: "),
                               _log_warning_name[warn]);
    else
        prefix = string_format(buffer, sizeof(buffer),
                               STRING_CONST("WARNING [%d]: "), warn);

    va_start(list, length);
    _log_outputf(context, ERRORLEVEL_WARNING, prefix.str, prefix.length, format,
                 length, list, stdout);
    va_end(list);
}
...
void log_errorf(hash_t context, error_t err, const char *format, size_t length,
    ...
    if (err < LOG_ERROR_NAMES)
        prefix =
            string_format(buffer, sizeof(buffer), STRING_CONST("ERROR [%s]: "),
                          _log_error_name[err]);
    else
        prefix = string_format(buffer, sizeof(buffer),
                               STRING_CONST("ERROR [%d]: "), err);

    va_start(list, length);
    _log_outputf(context, ERRORLEVEL_ERROR, prefix.str, prefix.length, format,
                 length, list, stderr);
    va_end(list);
}
...
void log_panicf(hash_t context, error_t err, const char *format, size_t length,
    ...
    if (err < LOG_ERROR_NAMES)
        prefix =
            string_format(buffer, sizeof(buffer), STRING_CONST("PANIC [%s]: "),
                          _log_error_name[err]);
    else
        prefix = string_format(buffer, sizeof(buffer),
                               STRING_CONST("PANIC [%d]: "), err);

    va_start(list, length);
    _log_outputf(context, ERRORLEVEL_PANIC, prefix.str, prefix.length, format,
                 length, list, stderr);
    va_end(list);
}
...
_log_error_contextf(hash_t context, error_level_t error_level, void *std,
...
void log_error_context(hash_t context, error_level_t error_level) {
    ...
    if (err_context && (log_suppress(context) < error_level)) {
        ...
        for (i = 0; i < err_context->depth; ++i, ++frame)
            _log_error_contextf(
                context, error_level,
                error_level > ERRORLEVEL_WARNING ? stderr : stdout,
                STRING_CONST("When %.*s: %.*s"), (int)frame->name.length,
...
void log_set_handler(log_handler_fn handler) {
