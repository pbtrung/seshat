...
static void md5_encode(unsigned char *dest, const uint32_t *src,
                       size_t length) {
    size_t i, j;
...
static void md5_decode(uint32_t *dest, const unsigned char *src,
                       size_t length) {
    size_t i, j;
    for (i = 0, j = 0; j < length; ++i, j += 4)
...
static void md5_transform(md5_t *digest, const unsigned char *buffer) {
    uint32_t a = digest->state[0], b = digest->state[1], c = digest->state[2],
             d = digest->state[3];
    uint32_t x[16];
...
md5_t *md5_digest(md5_t *digest, const void *buffer, size_t size) {
    ...
    memcpy(digest->buffer + index_buf, pointer_offset_const(buffer, index_in),
           size - index_in);
...
uint128_t md5_get_digest_raw(const md5_t *digest) {
    if (digest) {
        ...
        uint128_t val = {
            {byteorder_bigendian64(raw[0]), byteorder_bigendian64(raw[1])}
        };
#endif
        return val;
    }
