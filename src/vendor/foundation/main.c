...
static BOOL STDCALL _main_console_handler(DWORD control_type) {
    ...
    log_infof(0, STRING_CONST("Caught console control: %s (%lu)"), control_name,
              control_type);
    if (post_terminate) {
        /*lint -e{970} */
...
/*! Win32 UI application entry point, credits to Microsoft for ignoring yet
 * another standard... */
#if FOUNDATION_COMPILER_MSVC
FOUNDATION_API int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int);
#endif

int STDCALL WinMain(HINSTANCE instance, HINSTANCE previnst, LPSTR cline,
                    int cmd_show) {
    int ret;
...
#else
    {
        ...
        name = string_allocate_concat_varg(aname.length ? aname.str : "unknown",
                                           aname.length ? aname.length : 7,
                                           STRING_CONST("-"), STRING_ARGS(vstr),
                                           nullptr);
        ...
        if (app->exception_handler && !system_debugger_attached())
            ret = exception_try(main_run, 0, app->exception_handler,
                                STRING_ARGS(name));
        else
            ret = main_run(0);
...
#pragma clang diagnostic pop
    ...
    if (!(environment_application()->flags &
          (APPLICATION_UTILITY | APPLICATION_DAEMON))) {
...
#else
    {
        ...
        const application_t *app = environment_application();
        {
            ...
            name = string_allocate_format(STRING_CONST("%.*s-%.*s"),
                                          (int)aname.length, aname.str,
