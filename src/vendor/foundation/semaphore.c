...
bool semaphore_initialize(semaphore_t *semaphore, unsigned int value) {
    FOUNDATION_ASSERT(value <= 0xFFFF);
    *semaphore = CreateSemaphoreA(0, (long)value, 0xFFFF, 0); // lint !e970
    return (*semaphore) != 0 ? true : false;
}

bool semaphore_initialize_named(semaphore_t *semaphore, const char *name,
    ...
    *semaphore =
        CreateSemaphoreA(0, (long)value, 0xFFFF, namestr.str); // lint !e970
    return (*semaphore) != 0 ? true : false;
}
...
void *semaphore_event_handle(semaphore_t *semaphore) {
    return *semaphore;
}
...
bool semaphore_initialize(semaphore_t *semaphore, unsigned int value) {
    ...
    if (!semaphore->sem.unnamed) {
        ...
        log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                   STRING_CONST("Unable to initialize unnamed semaphore: %.*s"),
...
bool semaphore_initialize_named(semaphore_t *semaphore, const char *name,
    ...
    if (sem == SEM_FAILED) {
        ...
        log_errorf(
            ...
            STRING_CONST(
                "Unable to initialize named semaphore (sem_open '%.*s'): %.*s"),
    ...
    if (ret != 0) {
        ...
        if (errno != EINTR) {
            ...
            log_errorf(
                0, ERROR_SYSTEM_CALL_FAIL,
                STRING_CONST(
                    "Unable to wait for named semaphore '%.*s': %.*s (%d)"),
...
bool semaphore_try_wait(semaphore_t *semaphore, unsigned int milliseconds) {
    if (!semaphore->name.length) {
        long result = dispatch_semaphore_wait(
                          semaphore->sem.unnamed,
                          (milliseconds > 0)
                          ? dispatch_time(DISPATCH_TIME_NOW,
                                          1000000LL * (int64_t)milliseconds)
                          : DISPATCH_TIME_NOW);
        return (result == 0);
    }
#if FOUNDATION_PLATFORM_MACOS
    ...
    if (milliseconds > 0) {
        tick_t wakeup =
            time_current() +
            (((tick_t)milliseconds * time_ticks_per_second()) / 1000LL);
        do {
...
#pragma clang diagnostic ignored "-Wcast-qual"
    ...
    if (sem_init(&semaphore->unnamed, 0, value)) {
        ...
        string_const_t errmsg = system_error_message(err);
        log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                   STRING_CONST("Unable to initialize semaphore: %.*s (%d)"),
...
bool semaphore_initialize_named(semaphore_t *semaphore, const char *name,
    ...
    if (name && (length > 0) && (name[0] != '/'))
        semaphore->name =
            string_allocate_format(STRING_CONST("/%.*s"), (int)length, name);
    else
        semaphore->name = string_clone(name, length);
    semaphore->sem = 0;
    ...
    if (sem == SEM_FAILED) {
        ...
        string_const_t errmsg = system_error_message(err);
        log_errorf(0, ERROR_SYSTEM_CALL_FAIL,
                   STRING_CONST("Unable to initialize named semaphore "
                                "(sem_open '%.*s'): %.*s (%d)"),
                   STRING_FORMAT(semaphore->name), STRING_FORMAT(errmsg), err);
        return false;
    }
...
bool semaphore_try_wait(semaphore_t *semaphore, unsigned int milliseconds) {
    if (milliseconds > 0) {
        ...
        then.tv_nsec =
            (now.tv_usec * 1000) + (long)(milliseconds % 1000) * 1000000L;
