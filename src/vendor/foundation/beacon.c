...
beacon_t *beacon_allocate(void) {
    beacon_t *beacon =
        memory_allocate(0, sizeof(beacon_t), 0, MEMORY_PERSISTENT);
    beacon_initialize(beacon);
    return beacon;
}

void beacon_initialize(beacon_t *beacon) {
...
void beacon_finalize(beacon_t *beacon) {
#if FOUNDATION_PLATFORM_WINDOWS
...
void beacon_deallocate(beacon_t *beacon) {
    if (!beacon)
        return;
...
int beacon_wait(beacon_t *beacon) {
#if FOUNDATION_PLATFORM_WINDOWS
...
int beacon_try_wait(beacon_t *beacon, unsigned int milliseconds) {
    int slot = -1;
#if FOUNDATION_PLATFORM_WINDOWS
    unsigned int count = (unsigned int)beacon->count;
    unsigned int wait_status = WaitForMultipleObjects(
                                   count, (HANDLE *)beacon->all, FALSE, milliseconds);
    // WAIT_OBJECT_0 value is 0, so this checks range [WAIT_OBJECT_0,
    // WAIT_OBJECT_0+count)
    if (wait_status < count) {
        // Same behaviour as linux/bsd implementations, where auxiliary beacons
        // added  will remain fired after a beacon has seen it
...
void beacon_fire(beacon_t *beacon) {
#if FOUNDATION_PLATFORM_WINDOWS
...
int beacon_add_beacon(beacon_t *beacon, beacon_t *remote) {
...
void beacon_remove_beacon(beacon_t *beacon, beacon_t *remote) {
...
int beacon_add_handle(beacon_t *beacon, void *handle) {
...
void beacon_remove_handle(beacon_t *beacon, void *handle) {
    size_t islot;
...
int beacon_add_fd(beacon_t *beacon, int fd) {
    intptr_t handle = _get_osfhandle(fd);
    return beacon_add_handle(beacon, (void *)handle);
}

void beacon_remove_fd(beacon_t *beacon, int fd) {
    intptr_t handle = _get_osfhandle(fd);
    ...
    for (islot = 1; islot < beacon->count; ++islot) {
        if (beacon->all[islot] == fd) {
            ...
            if (islot < beacon->count) {
                ...
                epoll_ctl(beacon->poll, EPOLL_CTL_MOD, beacon->all[islot],
                          &event);
            }
        }
    }
...
int beacon_add_fd(beacon_t *beacon, int fd) {
    ...
    if (beacon->count < numslots) {
        beacon->all[beacon->count] = fd;
        struct kevent changes;
        EV_SET(&changes, fd, EVFILT_READ, EV_ADD, 0, 0,
               (void *)(uintptr_t)beacon->count);
        kevent(beacon->kq, &changes, 1, 0, 0, 0);
        return (int)beacon->count++;
    }
    return -1;
    ...
    for (islot = 1; islot < beacon->count; ++islot) {
        if (beacon->all[islot] == fd) {
            ...
            if (islot < beacon->count) {
                ...
                EV_SET(&changes, beacon->all[islot], EVFILT_READ, EV_ADD, 0, 0,
                       (void *)(uintptr_t)islot);
                kevent(beacon->kq, &changes, 1, 0, 0, 0);
            }
        }
    }
