/* bufferstream.c  -  Foundation library  -  Public Domain  -  2013 Mattias
...
stream_t *buffer_stream_allocate(void *buffer, unsigned int mode, size_t size,
                                 size_t capacity, bool adopt, bool grow) {
    stream_buffer_t *stream = memory_allocate(
                                  HASH_STREAM, sizeof(stream_buffer_t), 8, MEMORY_PERSISTENT);
    buffer_stream_initialize(stream, buffer, mode, size, capacity, adopt, grow);
    return (stream_t *)stream;
}

void buffer_stream_initialize(stream_buffer_t *stream, void *buffer,
                              unsigned int mode, size_t size, size_t capacity,
    ...
    if (!adopt && grow) {
        log_warn(
            0, WARNING_INVALID_VALUE,
            STRING_CONST("Cannot grow buffer streams that are not adopted"));
        grow = false;
    }
    ...
    stream->path = string_allocate_format(
                       STRING_CONST("buffer://0x%" PRIfixPTR), (uintptr_t)stream);
...
static size_t _buffer_stream_read(stream_t *stream, void *dest, size_t num) {
    ...
    if (num_read > 0) {
        memcpy(dest,
               pointer_offset(buffer_stream->buffer, buffer_stream->current),
               num_read);
...
static size_t _buffer_stream_write(stream_t *stream, const void *source,
                                   size_t num) {
    ...
    if (want > available) {
        if (buffer_stream->capacity >= (buffer_stream->current + want)) {
            ...
            available = want;
            buffer_stream->size = buffer_stream->current + want;
            buffer_stream->capacity = (buffer_stream->size < 1024)
                                      ? 1024
                                      : buffer_stream->size + 1024;
            // tail segment from current to size will be overwritten
            buffer_stream->buffer =
                memory_reallocate(buffer_stream->buffer,
                                  buffer_stream->capacity, 0, prev_capacity, 0);
        } else {
    ...
    if (num_write > 0) {
        memcpy(pointer_offset(buffer_stream->buffer, buffer_stream->current),
               source, num_write);
...
static void _buffer_stream_flush(stream_t *stream) {
    // lint --e{715, 818} stream unused and could be const, but it's really a
    // vtable function
...
static void _buffer_stream_truncate(stream_t *stream, size_t size) {
    ...
    if (buffer_stream->capacity >= size) {
        ...
        buffer_stream->buffer =
            memory_reallocate(buffer_stream->buffer, buffer_stream->capacity, 0,
...
static void _buffer_stream_seek(stream_t *stream, ssize_t offset,
    ...
    if (direction == STREAM_SEEK_CURRENT) {
        if (offset < 0)
            new_current = (abs_offset > buffer_stream->current)
                          ? 0
                          : (buffer_stream->current - abs_offset);
        else
        ...
        new_current = (offset < 0) ? buffer_stream->size - abs_offset
                      : buffer_stream->size;
