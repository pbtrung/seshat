...
read/write calls are blocking. Pipe streams are sequential (non-seekable). */

#include <foundation/platform.h>
#include <foundation/types.h>

/*! Allocate an unnamed pipe stream. Deallocate the stream with a call to
#stream_deallocate \return New pipe stream */
FOUNDATION_API stream_t *pipe_allocate(void);

/*! Initialize an unnamed pipe stream. Finalize the stream with a call to
#stream_finalize \param pipe Pipe stream */
