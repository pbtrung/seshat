...
count is 13, minimum bucket size is 8. Hash map should be deallocated with a
call to #hashmap_deallocate \param buckets Bucket count \param bucketsize Bucket
size \return New hash map */
FOUNDATION_API hashmap_t *hashmap_allocate(size_t buckets, size_t bucketsize);
...
count is 13, minimum bucket size is 8. Hash map should be finalized with a call
to #hashmap_finalize \param map Hash map to initialize \param buckets Bucket
count \param bucketsize Bucket size */
FOUNDATION_API void hashmap_initialize(hashmap_t *map, size_t buckets,
                                       size_t bucketsize);

/*! Finalize a hash map previously initialized with #hashmap_initialize and free
resources \param map Hash map */
FOUNDATION_API void hashmap_finalize(hashmap_t *map);

/*! Insert a new key-value mapping. Will replace any previously stored mapping
for the given key. \param map Hash map \param key Key \param value Value \return
Previously stored value, 0 if no value previously stored for key */
...
FOUNDATION_API size_t hashmap_size(hashmap_t *map);
