...
Bit buffer I/O for reading and writing tightly packed bits from/to a memory
buffer or a stream. The bit buffers support reading and writing integers up to
128 bits and floats up to 64 bits in one operation.

The bit buffer is not inherently thread safe, synchronization must be done by
caller in a multithreaded used case. */
...
FOUNDATION_API bitbuffer_t *bitbuffer_allocate_buffer(void *buffer, size_t size,
        bool swap);
...
\param size       Size of memory buffer (must be a multiple of 4 bytes)
\param swap       Byte order swap flag */
FOUNDATION_API void bitbuffer_initialize_buffer(bitbuffer_t *bitbuffer,
...
\param bitbuffer  Bit buffer object
\param stream     Stream for I/O */
FOUNDATION_API void bitbuffer_initialize_stream(bitbuffer_t *bitbuffer,
        stream_t *stream);
...
\return           Data read */
FOUNDATION_API uint32_t bitbuffer_read32(bitbuffer_t *bitbuffer,
        unsigned int bits);
...
\return           Data read */
FOUNDATION_API uint64_t bitbuffer_read64(bitbuffer_t *bitbuffer,
        unsigned int bits);
...
\return           Data read */
FOUNDATION_API uint128_t bitbuffer_read128(bitbuffer_t *bitbuffer,
        unsigned int bits);
...
FOUNDATION_API void bitbuffer_write32(bitbuffer_t *bitbuffer, uint32_t value,
                                      unsigned int bits);
...
FOUNDATION_API void bitbuffer_write64(bitbuffer_t *bitbuffer, uint64_t value,
                                      unsigned int bits);
...
FOUNDATION_API void bitbuffer_write128(bitbuffer_t *bitbuffer, uint128_t value,
                                       unsigned int bits);

/*! Write 32 bits of float data
\param bitbuffer  Bit buffer object
\param value      Data to write */
FOUNDATION_API void bitbuffer_write_float32(bitbuffer_t *bitbuffer,
        float32_t value);

/*! Write 64 bits of float data
\param bitbuffer  Bit buffer object
\param value      Data to write */
FOUNDATION_API void bitbuffer_write_float64(bitbuffer_t *bitbuffer,
        float64_t value);

/*! Align input to next even 32-bit boundary. Any remaining pending bit data is
discarded, a new 32 bit chunk is read from the buffer and bit pointer is set to
first bit in that chunk. If a full 32 bit chunk is available nothing is done,
unless force flag is set in which case the chunk is discarded and a new chunk is
read. \param bitbuffer  Bit buffer \param force      Force reading new data even
if full 32 bits are available */
FOUNDATION_API void bitbuffer_align_read(bitbuffer_t *bitbuffer, bool force);

/*! Force output of the current write chunk to the buffer. Unwritten bits are
padded with zero. If no pending data availble (no bits written since last chunk
output), nothing is done unless force flag is set in which case a full zero
chunk is written. \param bitbuffer  Bit buffer \param force      Force writing
data even if no bits are pending */
FOUNDATION_API void bitbuffer_align_write(bitbuffer_t *bitbuffer, bool force);

/*! Discard pending read data. This will restore the bit pointer to the start of
the current 32 bit chunk, effectively discarding the effect of previous reads on
the 32 bit chunk and allowing a re-read of the data. \param bitbuffer  Bit
buffer */
...
of the current 32 bit chunk, effectively discaring any previous writes of the 32
bit chunk and allowing a re-write of the data. \param bitbuffer  Bit buffer */
