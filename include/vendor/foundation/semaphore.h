...
FOUNDATION_API bool semaphore_initialize(semaphore_t *semaphore,
        unsigned int value);
...
\param value Initial value
\return true if successful, false if error */
FOUNDATION_API bool semaphore_initialize_named(semaphore_t *semaphore,
        const char *name, size_t length,
        unsigned int value);

/*! Finalize semaphore. The semaphore value must be >= than the value it was
created with, or it will be considered to be still in use (on OSX/iOS targets
this will cause a debug abort). \param semaphore Semaphore */
...
FOUNDATION_API bool semaphore_try_wait(semaphore_t *semaphore,
...
FOUNDATION_API void *semaphore_event_handle(semaphore_t *semaphore);
