...
MD5 message-digest algorithm. Inspired by Alexander Peslyak's public domain
implementation available at
http://openwall.info/wiki/people/solar/software/public-domain-source-code/md5

Normal use case is to first allocate/initialize the md5 block, then do any
...
/*! Finalize MD5 block previously initialized with #md5_initialize. After this
call the block may no longer be used until a new #md5_initialize call is made.
\param digest MD5 block */
...
\return MD5 block */
FOUNDATION_API md5_t *md5_digest(md5_t *digest, const void *buffer,
                                 size_t size);

/*! Finalize digest. Must be called between digesting data with #md5_digest
and getting the final message digest with #md5_get_digest/#md5_get_digest_raw.
If a new digest sequence is required the block must be re-initialized with a
call to #md5_initialize. \param digest MD5 block */
...
FOUNDATION_API string_t md5_get_digest(const md5_t *digest, char *str,
                                       size_t length);

/*! Get digest as raw 128-bit value. Before getting the raw digest the MD5 block
must be finalized with a call to #md5_digest_finalize. \param digest MD5 block
