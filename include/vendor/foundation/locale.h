...
/*! Define a locale with a four character code, two lowercase language
characters and two uppercase country characters, for example
LOCALE_DEF('e','n','U','S') \param x First language character \param y Second
language character \param z First country character \param w Second country
character */
#define LOCALE_DEF(x, y, z, w)                                                 \
    ((uint32_t)x | ((uint32_t)y << 8) | ((uint32_t)z << 16) |                  \
     ((uint32_t)w << 24))

/*! Define a language with a two character lowercase code, for example
LANGUAGE_DEF('e','n') \param x First character \param y Second character */
#define LANGUAGE_DEF(x, y) ((uint16_t)x | ((uint16_t)y << 8))

/*! Define a country with a two character uppercase code, for example
COUNTRY_DEF('U','S') \param x First character \param y Second character */
#define COUNTRY_DEF(x, y) ((uint16_t)x | ((uint16_t)y << 8))

/*! Define a locale from a compound language and country code, for example
<code>LOCALE_FROM_LANGUAGE_COUNTRY(LANGUAGE_ENGLISH,
COUNTRY_UNITED_STATES)</code> \param lang Language code \param country Country
code */
#define LOCALE_FROM_LANGUAGE_COUNTRY(lang, country)                            \
    ((uint32_t)(lang) | ((uint32_t)(country) << 16))
...
#define COUNTRY_SOUTH_GEORGIA_AND_THE_SOUTH_SANDWICH_ISLANDS                   \
    COUNTRY_DEF('G', 'S')
