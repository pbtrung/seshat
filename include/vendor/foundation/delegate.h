...
/*! macOS and iOS only. Start the main thread as a separate thread. The process
entry thread will go on and run the main Cocoa event loop. */
FOUNDATION_API void delegate_start_main_ns_thread(void);

/*! macOS and iOS only. Ensures all delegate classes and methods have references
to prevent from being stripped in optimization passes during compilation and
linking */
...
/*! macOS only. Get the currently assigned window from the delegate object
window outlet. \return Window object */
FOUNDATION_API void *delegate_window(void);

#if defined(__OBJC__) && !defined(FOUNDATION_NO_INTERFACE)

/*! Application delegate for foundation library projects. Assign the main
