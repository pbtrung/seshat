...
capacity to indicate the need for additional capacity for a full parse. Note
that string identifiers and values are in escaped form. \param buffer Data
buffer \param size Size of data buffer \param tokens Token array \param capacity
Capacity of token array (number of tokens) \return Number of parsed tokens, 0 if
error */
FOUNDATION_API size_t json_parse(const char *buffer, size_t size,
                                 json_token_t *tokens, size_t capacity);

/*! Parse a simplified JSON blob. Number of parsed tokens can be greater than
the supplied capacity to indicate the need for additional capacity for a full
parse. Not that string identifiers and values are in escaped form. \param buffer
Data buffer \param size Size of data buffer \param tokens Token array \param
capacity Capacity of token array (number of tokens) \return Number of parsed
tokens, 0 if error */
FOUNDATION_API size_t sjson_parse(const char *buffer, size_t size,
                                  json_token_t *tokens, size_t capacity);
...
FOUNDATION_API string_t json_unescape(char *buffer, size_t capacity,
...
FOUNDATION_API string_t json_escape(char *buffer, size_t capacity,
                                    const char *string, size_t length);
...
FOUNDATION_API size_t sjson_parse_path(const char *path, size_t length,
