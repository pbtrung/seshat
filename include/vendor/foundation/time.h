...
#time_ticks_per_second), measured from some system-specific base timestamp and
not in sync with other timestamps. \return Current timestamp */
...
/*! Get ticks as seconds (effectively calculating <code>ticks /
time_ticks_per_second()</code>). \param dt Deltatime in ticks \return Deltatime
in seconds */
