/* stacktrace.h  -  Foundation library  -  Public Domain  -  2013 Mattias
...
/*! Capture stack trace. If not supported, number of captured frames will be
zero. \param trace Stack trace buffer. Must be able to hold max_depth number of
frame pointers. \param max_depth Maximum call stack depth to capture \param
skip_frames Number of initial frames to skip before starting capture \return
Number of stack frames captured */
FOUNDATION_API size_t stacktrace_capture(void **trace, size_t max_depth,
        size_t skip_frames);
...
FOUNDATION_API string_t stacktrace_resolve(char *str, size_t capacity,
        void *const *trace, size_t max_depth,
        size_t skip_frames);
