...
Log output, filtering and context. Logging is centered around two concepts, a
context and a level. Log messages can be filtered on both, with a specific
threshold available for each context.

A context is identified by a hash value, normally a static string hash.

A log level is either debug, info, warning, error or panic (in increasing order
of significance).

Using different function calls for different log levels allows build-time contol
over code generation. Disabling log debug message build flag would make all
log_debug/log_debugf calls to be statically removed at compile time instead of
filtered at runtime, reducing binary size and call overhead. By enabling the
build flag and instead using a log context level filter a runtime selection of
log messages can be selected instead. */
...
FOUNDATION_API void log_debug(hash_t context, const char *msg,
                              size_t msg_length);
...
FOUNDATION_API void log_debugf(hash_t context, const char *format,
...
\param msg Log message
\param msg_length Length of message */
FOUNDATION_API void log_info(hash_t context, const char *msg,
                             size_t msg_length);
...
\param format_length Length of log message format specifier */
FOUNDATION_API void log_infof(hash_t context, const char *format,
                              size_t format_length, ...)
FOUNDATION_PRINTFCALL(2, 4);

/*! \fn void log_warn(hash_t context, warning_t warn, const char* msg, size_t
msg_length) Log an unformatted warning message in the specific context \param
context Log context \param warn Warning type \param msg Log message \param
msg_length Length of message */
FOUNDATION_API void log_warn(hash_t context, warning_t warn, const char *msg,
                             size_t msg_length);
...
\param format Log message format
\param format_length Length of log message format specifier */
FOUNDATION_API void log_warnf(hash_t context, warning_t warn,
                              const char *format, size_t format_length, ...)
FOUNDATION_PRINTFCALL(3, 5);

/*! \fn void log_error(hash_t context, error_t err, const char* msg, size_t
msg_length) Log an unformatted error message in the specific context. Will also
report the error code through error_report
\param context Log context
\param err Error code
\param msg Log message
\param msg_length Length of message */
FOUNDATION_API void log_error(hash_t context, error_t err, const char *msg,
                              size_t msg_length);
...
\param format Log message format
\param format_length Length of log message format specifier */
FOUNDATION_API void log_errorf(hash_t context, error_t err, const char *format,
                               size_t format_length, ...)
FOUNDATION_PRINTFCALL(3, 5);

/*! \fn void log_panic(hash_t context, error_t err, const char* msg, size_t
msg_length) Log an unformatted panic message in the specific context. Will also
report the error code through error_report
\param context Log context
\param err Error code
\param msg Log message
\param msg_length Length of message */
FOUNDATION_API void log_panic(hash_t context, error_t err, const char *msg,
                              size_t msg_length);
...
\param format Log message format
\param format_length Length of log message format specifier */
FOUNDATION_API void log_panicf(hash_t context, error_t err, const char *format,
                               size_t format_length, ...)
FOUNDATION_PRINTFCALL(3, 5);

/*! \fn void log_error_context(hash_t context, error_level_t error_level)
Log the current error context in the specified log context. The log output is
filtered at the given severity level \param context Log context \param
error_level Severity level */
FOUNDATION_API void log_error_context(hash_t context,
                                      error_level_t error_level);

/*! Get the currently set log handler
\return Log handler */
FOUNDATION_API log_handler_fn log_handler(void);

/*! Set log handler
\param handler New handler */
FOUNDATION_API void log_set_handler(log_handler_fn handler);
...
FOUNDATION_API bool log_stdout(void);
...
has no explicit supression level the default (0) context supression level will
be used. \param context Log context \param level Severity level to discard */
...
#define log_error_(...)                                                        \
    do {                                                                       \
        error_report(ERRORLEVEL_ERROR,                                         \
                     FOUNDATION_PREPROCESSOR_ELEM(1, __VA_ARGS__));            \
        FOUNDATION_UNUSED_VARARGS(__VA_ARGS__);                                \
    } while (0)
