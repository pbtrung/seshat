...
System queries and utility functions. All functions returning a string (const
char*) are thread safe and return a thread-local buffer that is valid until the
next call to a system function returning a string or the termination of the
thread. */

#include <foundation/platform.h>
#include <foundation/types.h>

/*! Get the current thread local system error code (errno in Posix, GetLastError
on Windows) \return Error code */
...
/*! Translate given error code into error message. System errno (unix variants)
or last error code (Windows) will be used if the given code is 0. \param code
Error code \return Error message (static buffer), empty string if no error */
...
FOUNDATION_API string_t system_hostname(char *buffer, size_t capacity);
...
FOUNDATION_API string_t system_username(char *buffer, size_t capacity);
...
FOUNDATION_API string_t system_locale_string(char *buffer, size_t capacity);

/*! Set the current locale (country & language).
\param locale Locale identifier (country & language) */
FOUNDATION_API void system_set_locale(uint32_t locale);

/*! Set the current device orientation. Usually only called internally by system
event listeners. \param orientation New device orientation */
FOUNDATION_API void
...
/*! Process system events. You should call this once every loop iteration in
 * your main loop. */
...
\param cancel_button Flag to trigger a cancel button as well as the default ok
button \return true if ok button pressed, false if cancel button pressed */
FOUNDATION_API bool system_message_box(const char *title, size_t title_length,
                                       const char *message,
...
FOUNDATION_API void system_thread_finalize(void);
