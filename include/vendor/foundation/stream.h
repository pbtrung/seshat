...
FOUNDATION_API stream_t *stream_open(const char *path, size_t length,
                                     unsigned int mode);
...
FOUNDATION_API bool stream_copy(stream_t *source, stream_t *destination);

/*! Deallocate stream previously allocated by any stream specific allocation
function or from a call to #stream_open. \param stream Stream */
...
/*! Finalize stream previously initialized by any stream specific initialization
function. \param stream Stream */
FOUNDATION_API void stream_finalize(stream_t *stream);

/*! Get current stream position.
\param stream Stream
\return Position in stream, 0 if error or invalid stream */
FOUNDATION_API size_t stream_tell(stream_t *stream);
...
FOUNDATION_API void stream_seek(stream_t *stream, ssize_t offset,
...
/*! Get the size (length) of the stream data in bytes, if available. If the size
can't be determined (sequential streams, like network sockets) the method will
return 0. \param stream Stream \return Stream size (length), or 0 if unknown */
FOUNDATION_API size_t stream_size(stream_t *stream);

/*! Set byte order of stream data. Only used in binary mode I/O.
\param stream Stream
\param byteorder New byte order */
FOUNDATION_API void stream_set_byteorder(stream_t *stream,
...
/*! Try to determine if stream is in binary or ASCII mode by reading the given
number of bytes and checking if all bytes are in printable ASCII range (byte
values [0x20,0x7e] and linefeed/tab). The method will restore the stream
position with a call to #stream_seek, so using this on a sequential stream is
not supported. \param stream Stream \param num Number of bytes to read */
FOUNDATION_API void stream_determine_binary_mode(stream_t *stream, size_t num);
...
/*! Query if stream is sequential (only forward seeking possible). Network
streams are generally sequential. \param stream Stream \return true if stream is
sequential, false if random access capable */
FOUNDATION_API bool stream_is_sequential(const stream_t *stream);

/*! Query if stream is reliable (data written guaranteed to be readable).
Network streams over an UDP socket are unreliable. \param stream Stream \return
true if stream is reliable, false if unreliable */
FOUNDATION_API bool stream_is_reliable(const stream_t *stream);

/*! Query if stream is in-order (data written guaranteed to be read back in the
same order). Network streams over an UDP socket are out-of-order. \param stream
Stream \return true if stream is reliable, false if unreliable */
FOUNDATION_API bool stream_is_inorder(const stream_t *stream);

/*! Query if bytes must be swapped when reading from stream (byte order of file
does not match byte order of system). \param stream Stream \return true if bytes
must be swapped, false if not */
...
\param stream Stream
\return Last modified timestamp, 0 if not open or date not meaningful for stream
type */
...
FOUNDATION_API size_t stream_read(stream_t *stream, void *buffer,
                                  size_t num_bytes);

/*! Read line of up to count characters, consuming but discarding delimiter,
reading into buffer. \param stream Stream \param dest Destination buffer, null
if input is to be ignored \param count Maximum number of characters to extract
\param delimiter Delimiter
\return Number of bytes read (including delimiter) */
FOUNDATION_API string_t stream_read_line_buffer(stream_t *stream, char *dest,
        size_t count, char delimiter);

/*! Read line as string, discarding delimiter. Allocates a new memory buffer for
the string, must be freed with a call to #string_deallocate \param stream Stream
\param delimiter Delimiter, default '\n'
\return String read, null string if error or if no bytes could be read from
stream */
...
\param stream Stream
\return Value read, 0 if error */
FOUNDATION_API uint128_t stream_read_uint128(stream_t *stream);

/*! Read 256-bit unsigned integer from stream.
\param stream Stream
\return Value read, 0 if error */
FOUNDATION_API uint256_t stream_read_uint256(stream_t *stream);

/*! Read 512-bit unsigned integer from stream.
\param stream Stream
\return Value read, 0 if error */
FOUNDATION_API uint512_t stream_read_uint512(stream_t *stream);

/*! Read UUID from stream.
\param stream Stream
\return Value read, 0 if error */
FOUNDATION_API uuid_t stream_read_uuid(stream_t *stream);
...
\param stream Stream
\return String, null string if error or if no bytes (or in binary mode, an empty
string) could be read from stream */
...
FOUNDATION_API string_t stream_read_string_buffer(stream_t *stream,
        char *buffer,
        size_t capacity);
...
FOUNDATION_API size_t stream_skip_whitespace(stream_t *stream);
...
FOUNDATION_API size_t stream_write(stream_t *stream, const void *buffer,
                                   size_t num_bytes);
...
\param stream Stream
\param data Unsigned integer to write */
FOUNDATION_API void stream_write_uint128(stream_t *stream, uint128_t data);

/*! Write 256-bit unsigned integer to stream.
\param stream Stream
\param data Unsigned integer to write */
FOUNDATION_API void stream_write_uint256(stream_t *stream, uint256_t data);

/*! Write 512-bit unsigned integer to stream.
\param stream Stream
\param data Unsigned integer to write */
FOUNDATION_API void stream_write_uint512(stream_t *stream, uint512_t data);

/*! Write UUID to stream.
\param stream Stream
\param data UUID to write */
FOUNDATION_API void stream_write_uuid(stream_t *stream, uuid_t data);
...
/*! If the stream is in text mode, write a separator character (whitespace) to
stream If the stream is in binary mode, ignore. \param stream Stream */
FOUNDATION_API void stream_write_separator(stream_t *stream);

/*! If the stream is in text mode, write an endline character to the stream and
flush. If the stream is in binary mode, only flush. \param stream Stream */
...
FOUNDATION_API uint256_t stream_sha256(stream_t *stream);
...
FOUNDATION_API uint512_t stream_sha512(stream_t *stream);

/*! Truncate stream to given size if it is larger, do nothing if smaller or
equal in size. \param stream Stream \param length New length of stream */
FOUNDATION_API void stream_truncate(stream_t *stream, size_t length);
...
FOUNDATION_API void stream_set_protocol_handler(const char *protocol,
        size_t length,
        stream_open_fn fn);
...
FOUNDATION_API stream_open_fn stream_protocol_handler(const char *protocol,
        size_t length);
