...
means than this thread interface you must remember to call #thread_exit on
thread termination to avoid memory leaks.
...
\return New thread  */
FOUNDATION_API thread_t *thread_allocate(thread_fn fn, void *data,
...
\param priority Thread priority
\param stacksize Thread stack size */
FOUNDATION_API void thread_initialize(thread_t *thread, thread_fn fn,
...
/* Finalize a thread
\param thread Thread */
FOUNDATION_API void thread_finalize(thread_t *thread);

/* Deallocate a thread
\param thread Thread */
FOUNDATION_API void thread_deallocate(thread_t *thread);

/*! Start execution of a thread. Must be paired with a corresponding call
to #thread_join
\param thread Thread
\return true if successful, false if failure */
FOUNDATION_API bool thread_start(thread_t *thread);

/*! Join an execution thread and free system resources. Must be paired with a
corresponding call to #thread_start
\param thread Thread
\return Thread exit code */
FOUNDATION_API void *thread_join(thread_t *thread);

/*! Query if thread has started execution
\param thread Thread
\return true if started, false if not */
FOUNDATION_API bool thread_is_started(const thread_t *thread);

/*! Query if thread is running
\param thread Thread
\return true if running, false if not */
FOUNDATION_API bool thread_is_running(const thread_t *thread);

/*! Signal thread (post thread semaphore)
\param thread Thread */
FOUNDATION_API void thread_signal(thread_t *thread);

/*! Query if calling thread is signalled
\return true if signalled, false if not */
FOUNDATION_API bool thread_wait(void);
...
\return true if signalled, false if not */
FOUNDATION_API bool thread_try_wait(unsigned int milliseconds);
...
/*! Get thread control block for the calling thread, 0 if not a foundation
thread \return Thread */
FOUNDATION_API thread_t *thread_self(void);

/*! Initialize on thread exit and aqcuire thread local resources. */
FOUNDATION_API void thread_enter(void);

/*! Finalize on thread exit and free thread local resources. */
FOUNDATION_API void thread_exit(void);

#if FOUNDATION_PLATFORM_ANDROID

/*! Android only. Attach the thread to the JVM to be able to do JVM calls
through JNI. Returns the JNI environment specific to the calling thread. \return
JNI environment (const struct JNINativeInterface**) for the calling thread, null
if error */
FOUNDATION_API void *thread_attach_jvm(void);

/*! Android only. Detach the thread from the JVM. No more calls to the JVM
through JNI can be made from this thread after a call to this function. */
