...
Small regular expression implementation matching a subset of Perl regular
expression syntax. <pre>    ^        Match beginning of string $        Match
end of string .        Match one character
...
FOUNDATION_API bool regex_parse(regex_t *regex, const char *pattern,
                                size_t length);

/*! Match input string with regular expression with optional captures. Note that
captures array might be modified and contain invalid data even if regex fails.
If the regex matches, the captures array will contain valid data. \param regex
Compiled expression \param input Input string \param inlength Length of input
string \param captures Result capture array, null if not wanted \param
maxcaptures Maximum number of captures \return true if string matches
