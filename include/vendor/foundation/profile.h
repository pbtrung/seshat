...
Profiling, collecting function level profiling data. Available if
#BUILD_ENABLE_PROFILE is set, otherwise all profiling function calls will
...
The string passed to the function must be constant over the execution of the
application (until #profile_finalize is called) and should identify the
application/build/machine or whatever you want it to identify.

Memory buffer should be large enough to hold data for ~100ms to avoid excessive
calls to output flush function. The profile subsystem will not allocate any
memory, it only uses the passed in work buffer. Recommended size is at least
256KiB. Maximum usable size is 4MiB, since the profiling system can only use 65k
blocks of 64 bytes each. \param identifier Application identifier \param length
Length of identifier \param buffer Work temporary buffer \param size Size of
work buffer */
FOUNDATION_API void profile_initialize(const char *identifier, size_t length,
                                       void *buffer, size_t size);

/*! Finalize profiling module and free resources. Will call the writer callback
with a null pointer and zero size to indicate end of transmission. */
FOUNDATION_API void profile_finalize(void);

/*! Control profiling operation. A true value enables all profiling calls, a
false value disables all calls. \param enable Enable flag */
FOUNDATION_API void profile_enable(bool enable);

/*! Set output function. The function will be called with blocks of data to
write to whatever output stream you want. \param writer New writer function */
...
Default is 100ms. Decresee time (increase rate) when passing a smaller buffer to
initialization, or increase time (decrease rate) if passing a larger buffer.
\param ms Wait time in milliseconds */
FOUNDATION_API void profile_set_output_wait(unsigned int ms);

/*! End a frame. Inserts a token into the profiling stream that identifies the
end of a frame, effectively grouping profile information together in a block.
...
#profile_begin_block must be matched with a call to #profile_end_block \param
message Block message (name) \param length Length of message */
...
/*! Insert log message. The string passed to this function must be constant
until the block is written to the output stream. \param message Message to
insert into the profile stream \param length Length of message */
FOUNDATION_API void profile_log(const char *message, size_t length);

/*! Lock notification. Call this method right before the thread tries to acquire
a lock on a mutually exclusive resource. The string passed to this function must
be constant until the block is written to the output stream. \param name Lock
name \param length Length of lock name */
FOUNDATION_API void profile_trylock(const char *name, size_t length);

/*! Lock notification. Call this method right after the thread has acquired a
lock on a mutually exclusive resource. The string passed to this function must
be constant until the block is written to the output stream. \param name Lock
name \param length Length of lock name */
FOUNDATION_API void profile_lock(const char *name, size_t length);

/*! Lock notification. Call this method right after the thread has released a
lock on a mutually exclusive resource. The string passed to this function must
be constant until the block is written to the output stream. \param name Lock
name \param length Length of lock name */
FOUNDATION_API void profile_unlock(const char *name, size_t length);

/*! Wait notification. Call this method right before the thread enters a wait
state on a mutually exclusive resource. The string passed to this function must
be constant until the block is written to the output stream. \param name Lock
name \param length Length of lock name */
...
on a mutually exclusive resource. The string passed to this function must be
constant until the block is written to the output stream. \param name Lock name
...
FOUNDATION_API string_const_t profile_identifier(void);
