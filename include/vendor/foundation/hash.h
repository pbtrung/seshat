...
FOUNDATION_API string_const_t hash_to_string(hash_t value);

#if BUILD_ENABLE_STATIC_HASH_DEBUG

static FOUNDATION_FORCEINLINE hash_t static_hash(const void *key, size_t len,
        hash_t value);

#else

#define static_hash(key, len, value)                                           \
    ((void)sizeof(key), (void)sizeof(len), (hash_t)(value))

#define hash_to_string(value) ((void)sizeof(value), (const char *)0)

#endif

/*! Declare a statically hashed string. If #BUILD_ENABLE_STATIC_HASH_DEBUG is
enabled in the build config this will allow the string to be reverse looked up
with hash_to_string. Static hash strings are usually defined by using the
hashify tool on a declaration file, see the hashstrings.txt and corresponding
hashstrings.h header \param key    Key string \param len    Length of key in
bytes \param value  Hash value */
#define static_hash_string(key, len, value) static_hash(key, len, (hash_t)value)

#if BUILD_ENABLE_STATIC_HASH_DEBUG

FOUNDATION_API void _static_hash_store(const void *key, size_t len,
                                       hash_t value);
